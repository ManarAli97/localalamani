...
class Controller
{
    ...
    public function permit($aclname = NULL, $aclgroup = NULL)
    {
        ...
        $stmtPermit = array();
        $stmt = $this->db->select("SELECT * from `permitgroup` WHERE `aclname`=:aclname AND `aclgroup`=:aclgroup ", array(':aclname' => $aclname, ':aclgroup' => $aclgroup));
        if (count($stmt) == 0) {
            ...
            $stmtPermit = $this->db->select("SELECT * from `permit` WHERE `idGroup`=:idGroup  AND `idParmit` =:idParmit  AND `permit` =:permit ", array(':idGroup' => $this->idGroup, ':idParmit' => $stmt[0]['id'], ':permit' => 1));
            if (!empty($stmtPermit)) {
                $stmtPermit = array_shift($stmtPermit);
            } else {
                $stmtPermit = array();
            }
        }
        if (in_array($this->userid, $this->idAdmin)) {
            ...
            if (in_array($this->idGroup, $stmtPermit)) {
                return true;
            } else {
                return false;
            }
...
// end functions lang
    ...
    function getNumberBill($l)
    {
        ...
        return   $this->runInDBTransaction(function () use ($l ) {
            ...
            $max = $this->db->prepare("SELECT  MAX(number_bill) as number_bill FROM cart_shop LIMIT 1");
            $max->execute();
            if ($max->rowCount() > 0)
            {
...
//	في حال حذفت فاتورة
    function clearBill($number_bill, $count)
    {
        if ($count == 1) {
            ...
            $stmt3 = $this->db->prepare("DELETE FROM `cart_shop` WHERE number_bill =? ");
    ...
    function get_location($model, $code, $color = null)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $stmtLocation = $this->db->prepare("SELECT id,location,quantity FROM `location` WHERE code=?  AND model=? AND quantity > 0 ");
    ...
    {
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE number_bill=?   ");
...
//	check quantity in cart
    ...
    function store_location($location,$model,$code)
    {
        ...
        $stmt->execute(array(trim(ltrim($location,',')),trim($model),$code));
        if ($stmt->rowCount() > 0)
        {
            $result=$stmt->fetch(PDO::FETCH_ASSOC);
...
// هاي الدالة تجريبية نستدعيها بشكل فردي
    ...
    function quantity_adjustment_send($id,$id_row,$table_name)
    {
        ...
        $stmt=$this->db->prepare("SELECT `id`, `code`,`price_dollars`,`range1`,`range2`, `quantity` FROM {$excel_tb} WHERE `code`=?  ");
...
function delete_type_device_send($id,$id_row,$table_name){
	...
	if($is_sync['check_is_sync'] || $id_sync > 0){
        ...
        if($id_code>0)
        {
        	...
        	$this->add_to_sync_log($id,$id_row,$table_name,'delete_type_device_send','error in id'.$id_item);
            return 0;
        }
    }
}
...
function set_quantity_order($table,$id_item,$code,$number)
{
    ...
    if ($table== 'mobile') {
        ...
        $stmt->execute(array($code));
		$this->Add_to_sync_schedule($code,$table,'quantity_adjustment',' ',' libs\Controller.php 4791  زيادة الطلب قيد المحاسبة او التجهيز'.$this->UserInfo($this->userid));
        ...
        if ($stmtCheckActveLocation->rowCount() > 0) {
            ...
            $stmt_ch_location = $this->db->prepare("SELECT  SUM(quantity) as q FROM  location  WHERE code =? AND  model=? ");
            $stmt_ch_location->execute(array($code, $table));
            if ($stmt_ch_location->rowCount() > 0) {
                $resultLocation = $stmt_ch_location->fetch(PDO::FETCH_ASSOC);
                if ($resultLocation['q'] <= 0) {
    ...
    function set_quantity_order_minus($table,$id_item,$code,$number)
    {
        ...
        $stmt = $this->db->prepare("UPDATE  `{$excel}` SET `quantity`=`quantity` + {$number} WHERE  `code`=?  ");
        $stmt->execute(array($code));
		$this->Add_to_sync_schedule($code,$table,'quantity_adjustment',' ',' libs\Controller.php 4865 نقصان الطلب فيد المحاسبة او التجهيز '.$this->UserInfo($this->userid));
    ...
    {
        $stmtCh=$this->db->prepare("SELECT *FROM serial_conform  INNER  JOIN  serial  ON serial.code = serial_conform.code  WHERE serial.code=?  AND  serial.model=? AND serial_conform.code=?  AND  serial_conform.model=? ");
        $stmtCh->execute(array($code,$model,$code,$model));
        if ($stmtCh->rowCount() <=0)
        {
