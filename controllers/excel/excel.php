...
class Excel extends Controller
{
    ...
    public function processing($model)
    {
        ...
        $columns = array(
            ...
            array( 'db' => 'range1', 'dt' => 6,
                'formatter' => function( $d, $row ) {
                    return number_format($d);
                }
            ),
            array( 'db' => 'range2', 'dt' => 7,
                'formatter' => function( $d, $row ) {
                    return number_format($d);
                }
                    ...
                    for ($row = 1; $row <= $highestRow; $row++) {
                        ...
                        if (count($rowData[0]) >= 4 ) {
                            ...
                            if (!empty($rowData[0][0])) {
                                ...
                                $stmt_code=$this->db->prepare("SELECT *FROM `{$code}` WHERE code=?");
...
// SSP::complex( $_GET, $sql_details, $table, $primaryKey, $columns )
    ...
    function inst_location($model,$code)
    {
        ...
        if($this->handleLogin())
        {
            ...
            $location=$_POST['setLocation']; /*  كمية */
            $indexLocation=$_POST['indexLocation']; /*  موقع */

            foreach ($location as $index => $quantity) {

                $stmt=$this->db->prepare('SELECT *FROM location WHERE code=? AND model=? AND location = ? AND id=? ');
                $stmt->execute(array($code,$model,$indexLocation[$index],$index));
                ...
                if ($quantity > $result['quantity'])
                {
                    ...
                    if ($stmtc->rowCount() > 0 )
                    {
                        $data = $this->db->update('location', array('quantity' => $quantity), "id={$index} AND code='{$code}' AND model='{$model}' AND location='{$indexLocation[$index]}' ");
