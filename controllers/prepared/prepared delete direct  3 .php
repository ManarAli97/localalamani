...
class preparedx extends Controller
{
	...
	public function index()
	{
		...
		{
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND direct <> 3  GROUP BY `number_bill` ORDER BY `number_bill`  ASC ");

		}else
		{
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND direct <> 3  AND `table`  IN ({$c})    GROUP BY `number_bill`  ORDER BY  `number_bill`   ASC ");
		}
		$stmt->execute();
		$count_active=array();
	...
	function load_order()
    {
        ...
        if ($this->admin($this->userid))
        {
            $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1     GROUP BY `number_bill` ORDER BY `number_bill`  ASC ");

        }else
        {
            $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1 AND `table`  IN ({$c})    GROUP BY `number_bill`  ORDER BY  `number_bill`   ASC ");
        }
        $stmt->execute();
        $count_active=array();
        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
	...
	public function prepared_made()
	{
		...
		$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE      `accountant` = 1 AND `prepared` = 2 AND  done_direct =0  AND `direct` < 2  GROUP BY `id_member_r`  ORDER BY `date_req` ASC ");
		$stmt->execute();
		$count_active=array();
		while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
		{
	...
	{
		$stmt = $this->db->prepare("SELECT * FROM  `cart_shop`  WHERE `id_member_r` = ? AND `buy` = 1 AND  `status` = 0 AND direct <> 3  AND number_bill <> 0");
	...
	public function all_notification_buy()
	{
		...
		if ($this->admin($this->userid)) {
			...
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND direct <> 3   AND number_bill <> 0 GROUP BY `number_bill`");
		}else
		{
//			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND `table`  IN ({$c})  AND ( `direct` < 2  OR `back_to`=1) GROUP BY `number_bill`");
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND direct <> 3  AND `table`  IN ({$c})   AND number_bill <> 0 GROUP BY `number_bill`");
...
//                    $stmtc->execute(array($code,$model));
	...
	function loadmore()
	{
		...
		if (   time() >= strtotime(date('Y-m-d').' 12:00:00 am')  &&  time()  < $toTime && $this->setting->get('hour') !=0 )
		{
			...
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE        `accountant` = 1 AND `prepared` = 2 AND    id_prepared =?    AND date_prepared between ? AND  ? GROUP BY `id_member_r`  ORDER BY `number_bill` ASC LIMIT $limit OFFSET $offset");
	...
	{
		$stmt = $this->db->prepare("SELECT  cancel,`date_req`,`date_d_r`,`id_member_r`,`delivery_user`,`number_bill`,`dollar_exchange`  FROM `cart_shop` WHERE `id_member_r` =?  AND  `prepared`=2   GROUP BY `number_bill`   ORDER BY `date` DESC ");
		...
		if ($stmt->rowCount() > 0) {
			...
			if($numberOrder <= $checkx)
			{
				...
				if ($stmt2->rowCount() > 0) {
					...
					$trace = new trace(); $trace->addtrace('cart_shop','تجهيز الفاتورة',json_encode($oldData),json_encode($newData),' تجهيز عناصر الفاتورة  من قبل    المجهز استخدام مواقع المادة ',$number_bill);
...
//					echo 'not_enough#'.$result['table'];//  الكمية المطلوبة اكبر من الموجودة في   الموقع
	...
	function  search()
	{
		if ($this->handleLogin())
		{
			...
			$q = '%' . $search . '%';
			$stmt = $this->db->prepare("SELECT register_user.id,register_user.name,register_user.phone,cart_shop.user_direct ,cart_shop.number_bill FROM `register_user` inner join cart_shop on cart_shop.id_member_r=register_user.id WHERE ( register_user.name LIKE ? OR  register_user.phone LIKE ? ) AND  cart_shop.accountant =1 AND  cart_shop.prepared =2 AND  cart_shop.cancel=0 GROUP BY cart_shop.id_member_r LIMIT 15");
	...
	function  searchActive()
	{
		if ($this->handleLogin())
		{
			...
			$search = strip_tags(trim($_GET['value']));
			$q = '%' . $search . '%';
			$stmt = $this->db->prepare("SELECT register_user.id,register_user.name,register_user.phone,cart_shop.user_direct ,cart_shop.number_bill  FROM `register_user` inner join cart_shop on cart_shop.id_member_r=register_user.id WHERE ( register_user.name LIKE ? OR  register_user.phone LIKE ? ) AND  cart_shop.accountant =1 AND cart_shop.prepared=1 AND `table` IN({$c}) AND cart_shop.cancel=0 GROUP BY cart_shop.id_member_r LIMIT 15");
