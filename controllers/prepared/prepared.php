...
class prepared extends Controller
{
	...
	public function index()
	{
		...
		{
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1     GROUP BY `number_bill`,`id_member_r` ORDER BY `number_bill`  ASC ");

		}else
		{
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1     AND `table`  IN ({$c})    GROUP BY `number_bill`,`id_member_r`  ORDER BY  `number_bill`   ASC ");
		}
		$stmt->execute();
		$count_active=array();
	...
	{
		$stmt = $this->db->prepare("SELECT * FROM  `cart_shop`  WHERE `id_member_r` = ? AND `buy` = 1 AND  `status` = 0   AND number_bill <> 0");
	...
	public function all_notification_buy()
	{
		...
		if ($this->admin($this->userid)) {
			...
			//$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND ( `direct` < 2  OR `back_to`=1)  GROUP BY `number_bill`");
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1      AND number_bill <> 0 GROUP BY `number_bill`");
		}else
		{
//			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1  AND `table`  IN ({$c})  AND ( `direct` < 2  OR `back_to`=1) GROUP BY `number_bill`");
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 1 AND `prepared` = 1     AND `table`  IN ({$c})   AND number_bill <> 0 GROUP BY `number_bill`");
	...
	{
		$stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill`,`top`,`dollar_exchange` FROM `cart_shop` WHERE `id_member_r` =? AND number_bill =? AND  `code`=?  AND prepared=1 LIMIT 1  ");
	...
	function return_order_minus($table,$code,$id_user)
	{
		...
		if ($this->handleLogin()) {
			...
			$id_row_tem=0;
			$stmt_count_n = $this->db->prepare("SELECT  *FROM `cart_shop` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ?  AND `number_bill` = ?   AND `buy` = 1 AND `number` = 1 AND `prepared`=1 ");
			...
			if ($stmt_count_n->rowCount() > 0 )
			{
				...
				$stmt_delete_last_one = $this->db->prepare("SELECT SUM(`number`)as number  FROM `cart_shop` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ? AND `number_bill` = ?   AND `buy` = 1 AND `prepared`=1  ");
				...
				if ($number == 1) {
					...
					$stmt = $this->db->prepare("DELETE   FROM `cart_shop` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ? AND `number_bill` = ?  AND `buy` = 1 AND `prepared`=1   LIMIT 1 ");
					$stmt->execute(array($table, $code, $id_user,$color,$number_bill));
					echo  0;

				}else if ($number > 1)
				{
					$stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` - 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `number_bill` = ?  AND `buy` = 1  AND number > 1 AND `prepared`=1 LIMIT  1  ");
            ...
            $stmt2 = $this->db->prepare("SELECT  SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ? AND `number_bill` = ?  AND `buy`= 1  AND `prepared`=1 GROUP BY `id_item`,`size`,`table`,`code`,`name_color` ORDER BY `id`  DESC  ");
			...
			if ($stmt2->rowCount() > 0) {
				...
				$trace = new trace(); $trace->addtrace('cart_shop','  المجهز - نقصان الفاتورة قبل التجهيز-رقم الفاتورة '.$number_bill,json_encode($oldData),json_encode($newData),' قد تحذف اخر مادة وتعتبر القيمة القديمة هي نفسها القمة المحذوفة',$number_bill);
	...
	function return_order_minus_after_accept($table,$code,$id_user)
	{
		...
		if ($this->handleLogin()) {
			...
			$stmt_count_n = $this->db->prepare("SELECT  *FROM `cart_shop` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ?  AND `number_bill` = ?   AND `buy` = 2 AND `number` = 1 AND `prepared`=2 ");
			$stmt_count_n->execute(array($table, $code, $id_user,$color,$number_bill));
			...
			if ($stmt_count_n->rowCount() > 0 )
			{
				...
				/*  trace Accountant Minus  */
				$stmt_retrieve_item=$this->db->prepare("SELECT *FROM `cart_shop` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ? AND `number_bill` = ? AND `buy` = 2 AND `number` = 1 AND `prepared`=2 LIMIT  1  ");
				$stmt_retrieve_item->execute(array($table, $code, $id_user,$color,$number_bill));
				$resultRI=$stmt_retrieve_item->fetch(PDO::FETCH_ASSOC);
				$accountant=new Accountant();
				$accountant->retrieve_item(array(
				...
				$stmt_delete_last_one = $this->db->prepare("SELECT SUM(`number`)as number  FROM `cart_shop` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ? AND `number_bill` = ?   AND `buy` = 2 AND `prepared`=2  ");
				$stmt_delete_last_one->execute(array($table, $code, $id_user,$color,$number_bill));
				$number= $stmt_delete_last_one->fetch(PDO::FETCH_ASSOC)['number'];

				if ($number == 1) {

					$stmt = $this->db->prepare("DELETE   FROM `cart_shop` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ? AND `number_bill` = ?  AND `buy` = 2 AND `prepared`=2   LIMIT 1 ");
					$stmt->execute(array($table, $code, $id_user,$color,$number_bill));
					$flag=1;

				}else if ($number > 1)
				{
					$stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` - 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `number_bill` = ?  AND `buy` = 2  AND number > 1 AND `prepared`=2 LIMIT  1  ");
					$stmt_sel->execute(array($table, $code, $id_user,$color,$number_bill));
					if ($stmt_sel->rowCount() < 1)
					{
						$stmt = $this->db->prepare("DELETE   FROM `cart_shop` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?   AND `number_bill` = ?  AND `buy` = 2  AND  `prepared`=2  LIMIT 1 ");
			...
			if ($flag==1) {
				...
				$trace = new trace(); $trace->addtrace('cart_shop','  المجهز - نقصان الفاتورة بعد التجهيز - رقم الفاتورة '.$number_bill,json_encode($oldData),json_encode($newData),' قد تحذف اخر مادة وتعتبر القيمة القديمة هي نفسها القمة المحذوفة',$number_bill);
	...
	function return_order_plus($table,$code,$id_user,$id_row)
	{
		...
		if ($this->handleLogin()) {
			...
			if ($r) {
				...
				$trace = new trace(); $trace->addtrace('cart_shop','تجهيز - زيادة الطلب',json_encode($oldData),json_encode($newData),'قام المجهز بزيادة الطلب - رقم الفاتورة  '.   $data['number_bill'],$number_bill);



                $stmt2 = $this->db->prepare("SELECT SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ? AND `name_color`=? AND `number_bill`=? AND `buy`= 1 GROUP BY `id_item`,`size`,`table`,`code` ORDER BY `id`  DESC  ");
				$stmt2->execute(array($table, $code, $id_user,$color,$number_bill));
				$res = $stmt2->fetch(PDO::FETCH_ASSOC);
	...
	function loadmore()
	{
		...
		if (   time() >= strtotime(date('Y-m-d').' 12:00:00 am')  &&  time()  < $toTime && $this->setting->get('hour') !=0 )
		{
			...
			$stmt = $this->db->prepare("SELECT register_user.*,cart_shop.user_direct,cart_shop.number_bill FROM `cart_shop` INNER JOIN  register_user ON register_user.id = cart_shop.id_member_r  WHERE      cart_shop.`accountant` = 1 AND cart_shop.`prepared` = 2 AND cart_shop.cancel=0 AND  cart_shop.done_direct =0     AND   cart_shop.id_prepared =?    AND cart_shop.date_prepared between ? AND  ? GROUP BY `id_member_r`      LIMIT $limit OFFSET $offset");
	...
	{
		$stmt = $this->db->prepare("SELECT    *  FROM `cart_shop` WHERE `id_member_r` =?  AND  `prepared`=2   GROUP BY `number_bill`   ORDER BY `date` DESC ");
        ...
        if ( $this->setting->get('prepared_serial_or_code',0) == 0 || $back==1){
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if ($stmt_serial->rowCount() > 0)
                {
                    ...
                    $stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=?  AND `number_bill`=? AND `accountant`=1 AND  `prepared` =1");
                    ...
                    if ($stmt2->rowCount() > 0) {
                        ...
                        $trace = new trace(); $trace->addtrace('cart_shop','تجهيز الفاتورة',json_encode($oldData),json_encode($newData),' تجهيز عناصر الفاتورة  من قبل   المجهز ',$number_bill);
		 ...
		 /*   $thisModel=$this->check_other_code_in_cart_shop($this->userid);
			$stmtAllcode=$this->db->prepare("SELECT `table`,code,name_color,id_member_r FROM `cart_shop` WHERE  number_bill=? AND `prepared`=1 AND `table` IN ({$thisModel})  GROUP BY  `table`,`code`,`color`  ");
...
//		$location=$_POST['alixcol'];
		...
		$stmt=$this->db->prepare("SELECT  `id`, `id_item`,`size`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`dollar_exchange` FROM `cart_shop` WHERE id_member_r=? AND `code`=? AND `number_bill`=? AND prepared = 1 ");
		$stmt->execute(array($id_user,$code,$number_bill));
		if ($stmt->rowCount() > 0) {
			$result=$stmt->fetch(PDO::FETCH_ASSOC);
			...
			if($numberOrder <= $checkx)
			{
				...
				if ($passLocation) {
                    $stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=?,location=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=? AND  `number_bill`=? AND `accountant`=1 AND `prepared`=1");
                    ...
                    if ($stmt2->rowCount() > 0) {
                        ...
                        $trace = new trace();
                        $trace->addtrace('cart_shop', 'تجهيز الفاتورة', json_encode($oldData), json_encode($newData), ' تجهيز عناصر الفاتورة  من قبل    المجهز استخدام مواقع المادة ', $number_bill);
					...
					if (isset($_POST['location'])) {
					   $location=$_POST['alixcol'];
						...
						if($numberOrder <= $checkx)
						{
							...
							$stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=?,`serial`=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=?  AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
...
//                                    echo 'hide_location#'.$result['table'];//  الكمية المطلوبة امبر من الموجودة في   الموقع
                            ...
                            if ($passLocation) {
                                ...
                                $stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=?,`serial`=?,`enter_serial`=?,location=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=?  AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
                                ...
                                if ($stmt2->rowCount() > 0) {
                                    $this->Add_to_sync_schedule($code, $table,'quantity_adjustment',' ',' controllers\prepared\prepared.php 1985 '.$this->UserInfo($this->userid));
                                    echo $code;
                                }
...
//					echo 'notFoundSerial#';
			...
			}else  if (isset($_POST['serial'])) // ملغي
			{
				...
				if ($c_serial =="true")
				{
					...
					$stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=?,`serial`=?,`enter_serial`=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=? AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
					$stmt2->execute(array(time(), $_SESSION['userid'],$serial,$enter_serial, $code , $id_user,$table,$result['id_item'],$number_bill));
					if ($stmt2->rowCount() > 0) {
                        $this->Add_to_sync_schedule($code,$table,'quantity_adjustment',' ',' controllers\prepared\prepared.php 2036 '.$this->UserInfo($this->userid));
						$stmtt=$this->db->prepare("SELECT *FROM `cart_shop` WHERE `number_bill`=?");
						$stmtt->execute(array($number_bill));
						$newData=array();
						while ($rowt = $stmtt->fetch(PDO::FETCH_ASSOC))
...
//                            echo 'hide_location#'.$result['table'];//  الكمية المطلوبة امبر من الموجودة في   الموقع
                    ...
                    if ($passLocation) {
                        $stmt_enter_serial = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=?,`enter_serial`=?,location=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=? AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
...
//					echo 'not_enough#'.$result['table'];//  الكمية المطلوبة اكبر من الموجودة في   الموقع
				...
				$stmt_enter_serial = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `id_prepared`=?,`enter_serial`=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=? AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
	...
	{
		$stmt=$this->db->prepare("SELECT *FROM `cart_shop` WHERE id_member_r=? AND id_item=? AND number_bill=? AND edit_bill=1 AND accountant=1");
	...
	function  search()
	{
		if ($this->handleLogin())
		{
			...
			$q = '%' . $search . '%';
			$stmt = $this->db->prepare("SELECT register_user.id,register_user.name,register_user.phone,cart_shop.user_direct ,cart_shop.number_bill FROM `register_user` inner join cart_shop on cart_shop.id_member_r=register_user.id WHERE ( register_user.name LIKE ? OR  register_user.phone LIKE ? OR cart_shop.number_bill=?) AND  cart_shop.accountant =1 AND  cart_shop.prepared =2 AND  cart_shop.cancel=0 GROUP BY cart_shop.id_member_r LIMIT 15");
	...
	function  searchActive()
	{
		if ($this->handleLogin())
		{
			...
			$search = strip_tags(trim($_GET['value']));
			$q = '%' . $search . '%';
			$stmt = $this->db->prepare("SELECT register_user.id,register_user.name,register_user.phone,cart_shop.user_direct ,cart_shop.number_bill  FROM `register_user` inner join cart_shop on cart_shop.id_member_r=register_user.id WHERE ( register_user.name LIKE ? OR  register_user.phone LIKE ? OR cart_shop.number_bill=?) AND  cart_shop.accountant =1 AND cart_shop.prepared=1 AND `table` IN({$c}) AND cart_shop.cancel=0 GROUP BY cart_shop.id_member_r LIMIT 15");
...
/*   because convert prepared on serial or code 2022-6-16
			$thisModel=$this->check_other_code_in_cart_shop($this->userid);
			$stmtAllcode=$this->db->prepare("SELECT `table`,code,name_color,id_member_r FROM `cart_shop` WHERE  number_bill=? AND `prepared`=1 AND `table` IN ({$thisModel})  GROUP BY  `table`,`code`,`color`  ");
			$stmtAllcode->execute(array($number_bill));
    ...
    public  function get_bill()
    {
        ...
        if ($this->handleLogin()) {
            ...
            $stmt = $this->db->prepare("SELECT register_user.* FROM `register_user` INNER JOIN cart_shop ON cart_shop.id_member_r = register_user.id WHERE cart_shop.number_bill = ? GROUP BY cart_shop.number_bill  LIMIT 1");
