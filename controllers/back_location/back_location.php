                ...
                if (file_exists($inputFileName)) {
                    ...
                    for ($row = 1; $row <= $highestRow; $row++) {
                        ...
                        if (count($rowData[0]) >= 3) {
                                ...
                                $stmtx1 = $this->db->prepare("SELECT SUM(number) as num  FROM `cart_shop`  WHERE `code`=?  AND `location`=?  AND `table` =?  AND cancel= 0  AND prepared=2 AND  date_d_r > 1637588022 GROUP BY code,`table`,id_item  ");
