...
class Accountant extends Controller
{
	...
	public function index()
	{
        ...
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   (`accountant` = 0 OR top= 1 ) AND `buy` <> 0 AND  `buy` < 2    AND `number_bill` <> 0  AND top =1 AND `cancel`=0 GROUP BY `number_bill` ,id_member_r   ");
		$stmt->execute();
		$count_active=array();
		...
		$stmt = $this->db->prepare("SELECT `cart_shop`.*,register_user.name,register_user.phone ,register_user.id  FROM `cart_shop` INNER JOIN register_user ON register_user.id = cart_shop.id_member_r WHERE   `cart_shop`.`accountant` = 0 AND `cart_shop`.`buy` <> 0 AND `cart_shop`.`buy` < 2    AND `cart_shop`.`number_bill` <> 0 AND `cart_shop`.top=0 AND `cart_shop`.`cancel`=0 GROUP BY `cart_shop`.`number_bill`,`cart_shop`.id_member_r  ORDER BY   `cart_shop`.`number_bill` ASC ");
		$stmt->execute();
		while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
		{
	...
	function load_order()
    {
        ...
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   (`accountant` = 0 OR top= 1 ) AND `buy` <> 0 AND  `buy` < 2    AND `number_bill` <> 0  AND top =1 AND `cancel`=0 GROUP BY `number_bill`    ");
        $stmt->execute();
        $count_active=array();
        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
        {
        ...
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 0 AND `buy` <> 0 AND  `buy` < 2    AND `number_bill` <> 0 AND top=0 AND `cancel`=0 GROUP BY `number_bill`  ORDER BY   `number_bill` ASC ");
        $stmt->execute();
        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
        {
            $stmt_user = $this->db->prepare("SELECT *FROM `register_user` WHERE id = ? ");
	...
	public function under_accounting()
	{
		...
		$this->checkPermit('view_request','accountant');
		$this->adminHeaderController($this->langControl('view_request'));
		$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE   `accountant` = 0   AND `buy` <> 0 AND  `buy` < 2    AND `number_bill` <> 0   AND `cancel`=0 GROUP BY `number_bill`  ORDER BY `top`,`id_member_r`,`date_req` DESC ");
		$stmt->execute();
		$count_active=array();
		while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
		{
	...
	{
        $stmt = $this->db->prepare("SELECT COUNT(*) as num FROM (SELECT COUNT(id) FROM `cart_shop` WHERE buy=1 AND number_bill <> 0 AND `accountant` = 0 AND `status` = 0 AND `cancel`=0  AND number_bill <> 0 GROUP BY number_bill,`id_member_r`) t");
        $stmt->execute();
	...
	public function auto_print_bill()
	{
		...
		$stmt = $this->db->prepare("SELECT COUNT(*) as num FROM (SELECT COUNT(id) FROM `cart_shop` WHERE direct <> 3 AND   buy=1 AND `accountant` = 0 AND `status` = 0  AND `auto_print`=0 AND `cancel`=0  AND number_bill <> 0 GROUP BY `id_member_r`,number_bill,`table`) t");
        }else {
            ...
            $c = implode(',', $categ);
            $stmt = $this->db->prepare("SELECT COUNT(*) as num FROM (SELECT COUNT(id) FROM `cart_shop` WHERE   direct <> 3 AND  `table` IN({$c}) AND buy=1 AND `accountant` = 0 AND `status` = 0  AND `auto_print`=0 AND cancel=0  AND number_bill <> 0 GROUP BY `id_member_r`,number_bill,`table`) t");
	...
	public function get_auto_print()
	{
        ...
        if ($this->admin($this->userid)) {
            $stmt = $this->db->prepare("SELECT id_member_r,number_bill FROM `cart_shop` WHERE   direct <> 3 AND   `auto_print`=0 GROUP BY id_member_r,number_bill LIMIT 1");
        }else{
            ...
            $c=implode(',',$categ);
            $stmt = $this->db->prepare("SELECT id_member_r,number_bill FROM `cart_shop` WHERE    direct <> 3 AND   `table` IN({$c}) AND  `auto_print`=0 AND cancel=0 GROUP BY id_member_r,number_bill,`table` LIMIT 1");
    ...
    public function all_notification_auto_print()
    {
        ...
        if ($this->admin($this->userid)) {
            $stmt = $this->db->prepare("SELECT COUNT(*) as num FROM (SELECT COUNT(id) FROM `cart_shop` WHERE  direct <> 3 AND   buy=1 AND `accountant` = 0 AND `status` = 0 AND `auto_print`=0  GROUP BY `id_member_r`,number_bill,`table`) t");
        }else{
            ...
            $c=implode(',',$categ);
            $stmt = $this->db->prepare("SELECT COUNT(*) as num FROM (SELECT COUNT(id) FROM `cart_shop` WHERE  direct <> 3 AND   `table` IN({$c}) AND buy=1 AND `accountant` = 0 AND `status` = 0 AND `auto_print`=0  GROUP BY `id_member_r`,number_bill,`table`) t");
    ...
    public function done_auto_print()
    {
        if ($this->handleLogin())
        {
            ...
            $stmt = $this->db->prepare("UPDATE   `cart_shop`  SET auto_print=1 WHERE id_member_r= ? AND number_bill =? ");
...
//		$this->checkPermit('view_order','accountant');
        ...
        $stmtOffer = $this->db->prepare("SELECT  cart_shop.*  FROM `cart_shop` WHERE `id_member_r` =? AND `number_bill`=? AND   `buy` = 1 AND `status` =0 AND `accountant`=0  AND id_offer <> 0 AND offers = 'offers' GROUP BY  `date_offer`  ORDER BY `id` DESC  ");
        ...
        $stmtOffer = $this->db->prepare("SELECT  cart_shop.*,SUM(number) as number FROM `cart_shop` WHERE `id_member_r` =? AND `number_bill`=? AND   `buy` = 1 AND `status` =0 AND `accountant`=0   AND id_offer = 0  AND offers = ''  GROUP BY  `id_item`,`table`,`code`,`color`,`number_bill`,price_type  ORDER BY `id` DESC  ");
        $stmtOffer->execute(array($id,$number_bill));
	...
	public  function order_miuns($id,$number_bill)
	{
		...
		if ($stmt2->rowCount() > 0 )
		{
			...
			$stmt3=$this->db->prepare("SELECT   *  FROM `cart_shop` WHERE `id_member_r`=? AND number_bill=? ");
	...
	{
		$stmt = $this->db->prepare("SELECT  cart_shop.*,SUM(`number`)as number FROM `cart_shop` WHERE `id_member_r` =? AND `number_bill`=? AND   `buy` = 1 AND `status` =0 AND `accountant`=0 GROUP BY `id_item`,`table`,`code`,`number_bill`,price_type,id_offer ORDER BY `id` DESC  ");
	...
	function return_order_minus($table,$code,$id_user)
	{
		...
		if ($this->handleLogin()) {
			...
			$stmt_count_n = $this->db->prepare("SELECT  *FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?    AND `number_bill`=? AND `buy` = 1 AND `accountant`=0 AND `number` = 1");
			...
			if ($stmt_count_n->rowCount() > 0 )
			{
				...
				$stmt_retrieve_item=$this->db->prepare("SELECT *FROM `cart_shop` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?  AND `number_bill` = ? AND `buy` = 1   LIMIT  1 ");
				...
				$stmt_delete_last_one = $this->db->prepare("SELECT SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ? AND `number_bill`=?  AND `buy` = 1 AND `accountant`=0");
				...
				if ($number == 1) {
					...
					$stmt = $this->db->prepare("DELETE   FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ? AND `number_bill`=? AND `buy` = 1 AND `accountant`=0  LIMIT 1 ");
					$stmt->execute(array($table, $code, $id_user,$color,$number_bill));
					echo  0;

				}else if ($number > 1)
				{
					$stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` - 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ? AND `number_bill`=? AND `buy` = 1  AND number > 1 AND `accountant`=0 LIMIT  1  ");
					...
					$stmt = $this->db->prepare("DELETE   FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?   AND `number_bill`=? AND `buy` = 1 AND `accountant`=0 LIMIT 1 ");
			...
			$trace = new trace(); $trace->addtrace('cart_shop','محاسبة',json_encode($oldData),json_encode($newData),  'تنقيص من الطلب',$number_bill);
			...
			$stmt2 = $this->db->prepare("SELECT  SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `number_bill`=? AND `buy`= 1 AND `accountant`=0 GROUP BY `id_item`,`size`,`table`,`code`,`name_color`  ORDER BY `id`  DESC  ");
	...
	function return_order_minus_after_accept($table,$code,$id_user)
	{
		...
		if ($this->handleLogin()) {
			...
			$stmt_count_n = $this->db->prepare("SELECT  *FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ?   AND `buy` = 1 AND `accountant`=1 AND `prepared`=1 AND `number_bill`=?  AND `number` = 1");
			...
			if ($stmt_count_n->rowCount() > 0 )
			{
                ...
                $stmt_totalc = $this->db->prepare("SELECT  *FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `number_bill`=?  LIMIT 1");
				...
				$stmt_delete_last_one = $this->db->prepare("SELECT SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ?   AND `buy` = 1 AND `accountant`=1 AND `prepared`=1 AND `number_bill`=?");
				$stmt_delete_last_one->execute(array($table, $code, $id_user,$color,$number_bill));
				$number= $stmt_delete_last_one->fetch(PDO::FETCH_ASSOC)['number'];

				if ($number == 1) {

					$stmt = $this->db->prepare("DELETE   FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `buy` = 1 AND `accountant`=1 AND `prepared`=1 AND `number_bill`=? LIMIT 1 ");
					$stmt->execute(array($table, $code, $id_user,$color,$number_bill));
					$flag=1;

				}else if ($number > 1)
				{
					$stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` - 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `buy` = 1  AND number > 1 AND `accountant`=1 AND `prepared`=1 AND `number_bill`=? LIMIT  1  ");
					$stmt_sel->execute(array($table, $code, $id_user,$color,$number_bill));
					if ($stmt_sel->rowCount() < 1)
					{
						$stmt = $this->db->prepare("DELETE   FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `buy` = 1 AND `accountant`=1 AND `prepared`=1 AND `number_bill`=? LIMIT 1 ");
					...
					$stmt = $this->db->prepare("DELETE   FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `buy` =2 AND `accountant`=1 AND `prepared`=1 AND `number_bill`=? LIMIT 1 ");
					$stmt->execute(array($table, $code, $id_user,$color,$number_bill));
					$flag=1;
				}
			...
			if ($flag==1) {
				...
				$trace = new trace(); $trace->addtrace('cart_shop','محاسبة',json_encode($oldData),json_encode($newData),'تنقيص الطلب بعد المحاسبة',$number_bill);
	...
	{
		$stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill`,`top`,`dollar_exchange` FROM `cart_shop` WHERE `id_member_r` =?  AND  `number_bill` =? AND `accountant`=0 GROUP BY `id_item`,`table`,`code`,`color` ORDER BY `id` DESC  ");
	...
	{
		$stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill`,`top`,`dollar_exchange` FROM `cart_shop` WHERE `id_member_r` =?  AND  `number_bill` =? AND `accountant`=1 GROUP BY `id_item`,`table`,`code`,`color` ORDER BY `id` DESC  ");
		$stmt->execute(array($id_member_r,$n_b));
		return $stmt;
	}
	...
	function return_order_plus($table,$code,$id_user)
	{
		...
		if ($this->handleLogin()) {
			...
			$stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` + 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ? AND  `name_color` = ?  AND  `number_bill` = ? AND `buy` = 1  AND `accountant`=0 LIMIT  1  ");
			$stmt_sel->execute(array($table, $code, $id_user,$color,$number_bill));
			...
			$trace = new trace(); $trace->addtrace('cart_shop','المحاسب زيادة الطلب -رقم الفاتورة '.$number_bill,json_encode($oldData),json_encode($newData),  '  زيادة الطلب ',$number_bill);
			$this->edit_bill($number_bill,$this->userid);
			...
			if ($stmt_sel->rowCount() > 0 ) {
               ...
               $resultIdItem= $stmttIdItem->fetch(PDO::FETCH_ASSOC);
				...
				$stmt2 = $this->db->prepare("SELECT SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ? AND `name_color`=?  AND `number_bill`=? AND `buy`= 1 AND `accountant`=0 GROUP BY `id_item`,`size`,`table`,`code`,`number_bill` ORDER BY `id`  DESC  ");
				$stmt2->execute(array($table, $code, $id_user,$color,$number_bill));
				$res = $stmt2->fetch(PDO::FETCH_ASSOC);
	...
	function processing_request($id)
	{
		if ($this->handleLogin()) {
			...
			if ($stmtChAcco->rowCount() > 0) {
				...
				$stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `status` = 1 ,  `accountant` = 1 , `prepared` = 1 , `date_accountant`= ? , `user_id`=?,`id_accountant_user`=?,`top`=0 ,auto_print=1 WHERE `number_bill` =? AND `buy` = ?  AND `id_member_r`=?  AND `date_req` IN ($date_req)  ");
				...
				if ($stmt2->rowCount() > 0) {
                    ...
                    $stmtPrint = $this->db->prepare("SELECT * FROM `cart_shop` WHERE  direct=2  AND  `accountant` = 1 AND `number_bill` = ? AND  id_member_r=? GROUP BY `id_member_r`,number_bill  ");
    ...
    function sunBill($id,$n_bill)
    {
        ...
        $price1_normal=0;
        $xp1Offer=0;
        $stmtOffer = $this->db->prepare("SELECT  cart_shop.* FROM `cart_shop` WHERE `id_member_r` =? AND `number_bill`=?  AND `accountant`=0  AND id_offer <> 0 AND offers = 'offers' GROUP BY  `date_offer`    ");
        $stmtOffer->execute(array($id, $number_bill));
        ...
        $stmtNotOffer = $this->db->prepare("SELECT  cart_shop.*,SUM(number) as number FROM `cart_shop` WHERE `id_member_r` =? AND `number_bill`=? AND    `accountant`=0   AND id_offer = 0  AND offers = ''  GROUP BY  `id_item`,`table`,`code`,`color`,`number_bill`,price_type     ");
...
//            }
	...
	function pay_bill()
	{
		if ($this->handleLogin()) {
            ...
            if ($id)
            {
                $stmtChAcco = $this->db->prepare("SELECT *FROM  `cart_shop`  WHERE `number_bill` =? AND `id_member_r`=? AND accountant=0 AND   `buy` = 1  AND cancel=0    ");
                $stmtChAcco->execute(array($number_bill,$id));

            }else
            {
                $stmtChAcco = $this->db->prepare("SELECT *FROM  `cart_shop`  WHERE `number_bill` =?  AND  accountant=0 AND   `buy` = 1  AND cancel=0    ");
                ...
                if ($stmtChAcco->rowCount() > 0) {
                    ...
                    $sum = (int)str_replace($this->comma, '', trim($price1));
                        ...
                        $stmtt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE `number_bill` = ? AND id_member_r=?  AND `$id`");
                    ...
                    while ($rowt = $stmtt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $stmt2 = $this->db->prepare("UPDATE `cart_shop` SET    `status` = 1 ,  `accountant` = 1 , `prepared` = 1 , `date_accountant`= ? , `user_id`=?,`id_accountant_user`=?,`top`=0,auto_print=1  WHERE `number_bill` =? AND `buy` = ?  AND `id_member_r`=?    ");
                        $stmt2->execute(array(time(), $this->userid, $this->userid, $number_bill, 1, $id));
	...
	function loadmore()
	{
		...
		{
			$stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE    `accountant` = 1     AND date_accountant between ? AND  ? GROUP BY `id_member_r`   LIMIT $limit OFFSET $offset");
	...
	{
		$stmt = $this->db->prepare("SELECT  `date_req`,`date_d_r`,`id_member_r`,`delivery_user`,`number_bill` ,`dollar_exchange`,`date_accountant`,`id_accountant_user` FROM `cart_shop` WHERE `id_member_r` =?  AND  `accountant`=1  GROUP BY `number_bill`   ORDER BY `date_accountant` DESC ");
	...
	{
		$stmt = $this->db->prepare("SELECT  cart_shop.*,SUM(`number`)as number FROM `cart_shop` WHERE `id_member_r` =?  AND   `accountant` = 1     AND `number_bill`= ? GROUP BY `id_item`,`table`,`code`,`number_bill`,price_type,id_offer  ORDER BY `id` DESC  ");
		$stmt->execute(array($id_member_r,$number_bill));
		return $stmt;
	}
		...
		if ($this->handleLogin()) {
			...
			if ($cat =='savers')
			{
				...
				if ($stmtCode_network->rowCount() > 0) {
					...
					$stmt_order = $this->db->prepare("SELECT   SUM(`number`)as num ,`dollar_exchange` FROM `cart_shop` WHERE `code` =?  AND `name_color` =?  AND `buy` = 1 AND `status` =0   AND `table`='product_savers' ");
	...
	function processing_request_rejected($id,$number_bill)
	{
		...
		if ($this->handleLogin()) {
			...
			$stmt_c = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`dollar_exchange` FROM `cart_shop` WHERE `id_member_r` =?  AND  `number_bill` =?  AND `buy` = 1 AND `accountant`=0 AND   `prepared`=0    GROUP BY `id_item`,`table`,`code`   ");
...
//                }
			...
			$stmt = $this->db->prepare("UPDATE `cart_shop` SET `cancel` = 1  ,`status` = 2  , `buy` = 3, `why_rejected` = ?  , `date_d_r`= ? , `user_id`=?,`id_accountant_user`=?  WHERE  `buy` = ? AND `id_member_r`=? AND `number_bill`=? AND accountant = 0");
			...
			$trace = new trace(); $trace->addtrace('cart_shop','محاسبة',json_encode($oldData),json_encode($newData),$this->userid . 'الغاء طلب ',$number_bill);
...
*/
	...
	function  search()
	{
		if ($this->handleLogin())
		{
			...
			$q = '%' . $search . '%';
			$stmt = $this->db->prepare("SELECT register_user.id,register_user.name,register_user.phone,cart_shop.id_accountant_user,cart_shop.number_bill  FROM `register_user` inner join cart_shop on cart_shop.id_member_r=register_user.id WHERE ( register_user.name LIKE ? OR  register_user.phone LIKE ? ) AND  cart_shop.accountant =1  AND cart_shop.cancel=0  GROUP BY cart_shop.id_member_r order by cart_shop.number_bill DESC  LIMIT 15");
