...
// SQL server connection information
		...
		$join = "inner JOIN register_user ON register_user.id = cart_shop.id_member_r  left JOIN user ON user.id = cart_shop.id_accountant_user ";
		if (empty($from_date_stm) && empty($to_date_stm))
		{
			$whereAll = array("{$table}.id_accountant_user ={$id}","accountant=1","cancel=0");

		}else{
			$whereAll = array("{$table}.id_accountant_user ={$id}","accountant=1","cancel=0","cart_shop.date_accountant BETWEEN {$from_date_stm} AND {$to_date_stm}");

		}
		$group="GROUP BY  cart_shop.number_bill";
	...
	{
		$stmt = $this->db->prepare("SELECT *FROM cart_shop WHERE `id_member_r`=? AND `id_accountant_user` =? AND `number_bill` =? ");
	...
	function account_billAll($id,$date,$todate)
	{
		...
		if ($date==null)
		{
			...
			$stmt =$this->db->prepare("SELECT   `number_bill`  FROM  `cart_shop` WHERE  `user_id`  = ?  AND  `accountant` = 1   AND cancel=0 GROUP BY `number_bill` ");
			$stmt->execute(array($id));
		...
		}else
		{
			...
			$stmt =$this->db->prepare("SELECT   `number_bill`  FROM  `cart_shop` WHERE  `user_id`  = ?  AND  `accountant` = 1  AND cancel=0 AND `date_accountant`  between ? AND ? GROUP BY `number_bill` ");
	...
	{
		$stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill`,`top`,`dollar_exchange` FROM `cart_shop` WHERE `user_id` =? AND `number_bill`=?  AND   `accountant`=1 AND cancel=0 GROUP BY `id_item`,`table`,`code`,`color` ORDER BY `id` DESC  ");
	...
	function number_billAll($id,$date,$todate)
	{
		...
		if ($date==null)
		{
			$stmt =$this->db->prepare("SELECT COUNT(number_bill)  as number_bill FROM ( SELECT COUNT(`number_bill`) as number_bill FROM  `cart_shop` WHERE  `id_accountant_user`  = ? AND `accountant` =1    GROUP BY number_bill  ) number_bill   ");
			$stmt->execute(array($id));
		}else
		{
			$fromDate=strtotime($date);
			...
			$stmt =$this->db->prepare("SELECT COUNT(number_bill)  as number_bill FROM ( SELECT COUNT(`number_bill`) as number_bill FROM  `cart_shop` WHERE  `id_accountant_user`  = ? AND `accountant` = 1  AND `date`  between ? AND ?  GROUP BY number_bill) number_bill ");
			$stmt->execute(array($id,$fromDate,$toDate));
