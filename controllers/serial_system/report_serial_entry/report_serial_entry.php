...
function data_code($code,$model)
{
    ...
    }else
    {
        ...
        $stmt=$this->db->prepare("SELECT  {$model}.title,{$color}.color,{$color}.img,{$code_table}.size,{$code_table}.code,{$excel}.quantity FROM {$code_table} INNER JOIN  {$color} ON  {$color}.id={$code_table}.id_color INNER JOIN  {$model} ON {$model}.id={$color}.id_item left JOIN  {$excel} ON {$excel}.code={$code_table}.code   WHERE {$code_table}.code=? LIMIT  1");
        $stmt->execute(array($code));
        $result=$stmt->fetch(PDO::FETCH_ASSOC);
    }
