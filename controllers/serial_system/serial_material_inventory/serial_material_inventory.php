...
trait serial_material_inventory
{
    ...
    function get_location_serial ($code,$model)
    {
            ...
            $stmt  = $this->db->prepare("SELECT    serial,(SELECT    GROUP_CONCAT(location SEPARATOR ',')  FROM serial  WHERE code=? AND  model=?  AND location <>'' )   as location  FROM serial  WHERE code=? AND  model=?  AND location <>'' GROUP BY serial ,code");
