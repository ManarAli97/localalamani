    ...
    function code($model)
    {
         ...
         }else
         {
            ...
            }else
            {
             ...
             $stmt=$this->db->prepare("SELECT {$code_table}.code,{$code_table}.size,{$color}.color,{$color}.id_item FROM {$code_table} INNER JOIN {$color} ON {$color}.id=code.id_color WHERE {$code_table}.code=? LIMIT 1");
             $stmt->execute(array($code));
            if ($stmt->rowCount() > 0)
            {
                $result=$stmt->fetch(PDO::FETCH_ASSOC);
