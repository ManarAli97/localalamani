...
class statistics extends Controller
{
    ...
    function index()
    {
		...
		$stmt_total_req = $this->db->prepare("SELECT  COUNT(*)  FROM `cart_shop` WHERE `date_req` between ? AND  ?  GROUP BY `number_bill`  ");
        ...
        $stmt_total_req2 = $this->db->prepare("SELECT  COUNT(*)  FROM `cart_shop`  WHERE `buy` = 2  AND    `date_req` between ? AND  ?   GROUP BY `number_bill`  ");
        ...
        $stmt_total_req3 = $this->db->prepare("SELECT  COUNT(*)  FROM `cart_shop`  WHERE (`buy` = 3 OR cancel = 1)  AND    `date_req` between ? AND  ? GROUP BY `number_bill`  ");
        ...
        $stmt_total_req4 = $this->db->prepare("SELECT  SUM(`number`)  AS `number` FROM `cart_shop`  WHERE  `buy` = 2  AND    `date_req` between ? AND  ?   GROUP BY `number_bill`  ");
