...
function done_delivery()
{
    ...
    if ($this->handleLogin())
    {
        $this->checkPermit('delivered','delivery_user');
          ...
          $stmt=$this->db->prepare("UPDATE `{$this->cart_shop}` SET `delivered`=1 , `delivered_date` = ?  WHERE `id_member_r`= ? AND  `date_d_r`=? AND `number_bill` = ? AND `delivered` = 0 AND  `number_bill` <> 0" );
