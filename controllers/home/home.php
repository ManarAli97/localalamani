...
class Home extends Controller
{
    ...
    function cart_shop()
    {
      ...
      // $stmt=$this->db->prepare("SELECT crystal_bill, number_bill  FROM crystal_bill GROUP BY number_bill    ");
        $stmt=$this->db->prepare("SELECT crystal_bill.crystal_bill, crystal_bill.number_bill  FROM `crystal_bill` LEFT JOIN cart_shop  ON cart_shop.crystal_bill = crystal_bill.crystal_bill WHERE cart_shop.crystal_bill IS NULL GROUP BY crystal_bill.number_bill    ");
        $stmt->execute();
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {

            $stmtup=$this->db->prepare("UPDATE  cart_shop SET  crystal_bill =? WHERE number_bill=?");
    ...
    function cart_shop_group()
    {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $stmtup=$this->db->prepare("UPDATE  cart_shop SET  group_bill =? WHERE number_bill=?");
    ...
    function index()
    {
        ...
        $stmt=$this->db->prepare("SELECT `date_accountant`,`dollar_exchange` FROM `cart_shop` WHERE `buy`=2 ORDER BY `date_accountant` DESC LIMIT 1 ");
        $stmt->execute();
        ...
        foreach ($day as $key => $d)
        {
            ...
            $stmt_done = $this->db->prepare("SELECT   SUM(`number`) as num ,`dollar_exchange` FROM `cart_shop` WHERE    `accountant`=1 AND  cancel=0  AND  ( `date_accountant`  between  ? AND  ? )");
        ...
        foreach ($day as $key => $d)
        {
            ...
            $stmt_1 = $this->db->prepare("SELECT  *FROM `cart_shop` WHERE     `accountant`=1 AND  cancel=0  AND   ( `date_accountant`  between  ? AND  ? )");
