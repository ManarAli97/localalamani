...
use  search_rewind;
	...
	function details($number_bill)
	{
		...
		$stmt=$this->db->prepare("SELECT review_item.*,cart_shop.price_dollars,cart_shop.image FROM review_item     INNER JOIN cart_shop ON cart_shop.id=review_item.id_cart WHERE   number_bill_new=?  AND review_item.cancel=0 ");
    ...
    public function processing_bill_materials_crystal_not_enter($number,$from_date_stm=null,$to_date_stm=null)
    {
        ...
        $columns = array(
            ...
            array('db' => $table.'.number_bill_new', 'dt' => 5,
                'formatter' => function ($d, $row) {
                    return "<div class='text-right'>{$this->customerInfo($row[7])} ,{$this->UserInfo($row[5])} ,{$this->UserInfo($row[6])} ,{$d}   </div> ";
                }
...
// SQL server connection information
    ...
    function details2($number_bill)
    {
        ...
        $stmt=$this->db->prepare("SELECT review_item.*,cart_shop.price_dollars,cart_shop.image FROM review_item     INNER JOIN cart_shop ON cart_shop.id=review_item.id_cart WHERE   number_bill_new=?  AND review_item.cancel=2");
        $stmt->execute(array($number_bill));
        $bill=array();
