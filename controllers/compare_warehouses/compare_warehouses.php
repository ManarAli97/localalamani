...
// SQL server connection information
    ...
    function sale($code,$color)
    {
        ...
        $stmt= $this->db->prepare("SELECT   SUM(`number`)as num  FROM `cart_shop` WHERE `code` = ? AND `name_color`=? AND `accountant` = 1  AND `buy` = 2 AND cancel=0   ");
    ...
    function sale_cover($code,$model)
    {
        ...
        $stmt= $this->db->prepare("SELECT   SUM(`number`)as num  FROM `cart_shop` WHERE `code` = ? AND `table`=? AND `accountant` = 1  AND `buy` = 2 AND cancel=0   ");
    ...
    function booked_up($code,$color)
    {
        ...
        $stmt= $this->db->prepare("SELECT   SUM(`number`)as num  FROM `cart_shop` WHERE `code` = ? AND `name_color`=? AND  `buy` = 1   AND cancel=0 ");
        $stmt->execute(array($code, $color));
    ...
    function booked_up_cover($code,$model)
    {
        ...
        $stmt= $this->db->prepare("SELECT   SUM(`number`)as num  FROM `cart_shop` WHERE `code` = ? AND `table`=? AND  `buy` = 1   AND cancel=0 ");
        $stmt->execute(array($code, $model));
        $booked = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($booked['num'])
        {
    ...
    public function processing_cover($brand=0,$series=0,$id_device=0,$cov=0,$typ=0,$feat=0)
    {
        ...
        if ($this->setting->get('type_cover',1) == 1) {
            ...
            $columnsx = array(
                ...
                array('db' => $excel . '.quantity', 'dt' => 10),
                array('db' => $this->table . '.code', 'dt' => 11,
            ...
            $columnsx = array(
                ...
                array('db' => 'product_savers.feature_cover', 'dt' => 6,
                    'formatter' => function ($d, $row) {
                        ...
                        return "<p>{$this->name_cover_material($row[35])}</p> <p>{$this->name_type_cover($row[36])}</p> <p>{$this->name_feature_cover($d)}</p>";
                    }

                ),

                array('db' => $excel . '.quantity', 'dt' => 7),
                array('db' => $this->table . '.code', 'dt' => 8,
                ...
                array('db' => 'product_savers.cover_material', 'dt' => 35),

                array('db' => 'product_savers.type_cover', 'dt' => 36),
...
// SQL server connection information
            ...
            try {
                ...
                if (file_exists($inputFileName)) {
                    ...
                    for ($row = 1; $row <= $highestRow; $row++) {
                        ...
                        $stmt = $this->db->prepare("INSERT INTO `material` (`category`,`code`,`name`,`latiniin`,`date`,id_device,type,brand,series) VALUES(?,?,?,?,?,?,?,?,?)");
                        $stmt->execute(array('savers',$rowData[0][0], $rowData[0][1], $rowData[0][2],  time(),$data['id_device'],$data['type_cover'],$data['brand'],$data['series']));
                        $lastId=$this->db->lastInsertId();
    ...
    function delete_all($model)
    {
        if ($this->handleLogin()) {
            ...
            $stmt = $this->db->prepare("DELETE FROM `material` WHERE category=?  ");
