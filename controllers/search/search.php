        ...
        if (isset($_GET['submit'])) {
            ...
            if ($typeCat == 'all') {
				...
				if ($this->thisCatg('network')) {
                    ...
                    $network = new network();
                    $stmtcode = $this->db->prepare("SELECT   network.`price_cuts`, network.`id`,network.`title`,network.`bast_it`,network.`price_dollars`,network.description,network.cuts,excel_network.price_dollars as excel_price_dollars  ,excel_network.quantity,color_network.img,color_network.code_color,code_network.size,code_network.code,excel_network.wholesale_price,excel_network.wholesale_price2,excel_network.cost_price   FROM code_network inner JOIN color_network ON code_network.id_color = color_network.id inner JOIN network ON color_network.id_item = network.id  inner JOIN excel_network ON excel_network.code = code_network.code WHERE  ( network.title LIKE ? OR  network.tags LIKE ?  OR code_network.code=?)    AND network.`active` = 1  AND excel_network.quantity > 0");
                    $stmtcode->execute(array($q,$q,str_replace(array(' ','%'),'',trim($q))));
                    if ($stmtcode->rowCount() > 0) {
                        while ($row = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
...
//  end all search
                ...
                if ($type == 'network'  && $this->thisCatg('network')) {
                    ...
                    $stmtcode = $this->db->prepare("SELECT   network.`price_cuts`, network.`id`,network.`title`,network.`bast_it`,network.`price_dollars`,network.description,network.cuts,excel_network.price_dollars as excel_price_dollars  ,excel_network.quantity,color_network.img,color_network.code_color,code_network.size,code_network.code,excel_network.wholesale_price,excel_network.wholesale_price2,excel_network.cost_price  FROM code_network inner JOIN color_network ON code_network.id_color = color_network.id inner JOIN network ON color_network.id_item = network.id  inner JOIN excel_network ON excel_network.code = code_network.code WHERE  ( network.title LIKE ? OR  network.tags LIKE ?  OR code_network.code=?)    AND network.`active` = 1    AND network.`id_cat` IN ({$Id_cat})  AND excel_network.quantity > 0");
                    $stmtcode->execute(array($q,$q,str_replace(array(' ','%'),'',trim($q))));
                    if ($stmtcode->rowCount() > 0) {
                        while ($row = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
    ...
    function  smartsearch()
    {
         ...
         if (!empty($val)) {
             ...
             if ($cat == 'all') {
				 ...
				 if ($this->thisCatg('network')) {
                     $stmtcode = $this->db->prepare("SELECT   network.`id`,network.`title`,network.`bast_it`   FROM code_network inner JOIN color_network ON code_network.id_color = color_network.id inner JOIN network ON color_network.id_item = network.id  inner JOIN excel_network ON excel_network.code = code_network.code WHERE  ( network.title LIKE ? OR  network.tags LIKE ?  OR code_network.code=?)    AND network.`active` = 1   AND excel_network.quantity > 0  GROUP BY network.title ORDER BY network.bast_it DESC LIMIT 10");
                     $stmtcode->execute(array($q,$q,str_replace(array(' ','%'),'',trim($q))));
                     if ($stmtcode->rowCount() > 0) {
                         while ($row = $stmtcode->fetch(PDO::FETCH_ASSOC)) {
                             if ($row['bast_it']==1)
