        ...
        if ($this->handleLogin()) {
            ...
            if ($cat =='accessories') {
                ...
                if ($stmtCode_accessories->rowCount() > 0) {
                    ...
                    $stmt_order = $this->db->prepare("SELECT   SUM(`number`)as num,`dollar_exchange`  FROM `cart_shop` WHERE `code` =?  AND `name_color` =?  AND `buy` = 1 AND `status` =0  AND `table`='accessories' ");
