...
class check_accessories extends Controller
{
    ...
    $columns = array(
        ...
        array( 'db' =>   'cart_shop.number_bill', 'dt' =>  1 ,
...
// SQL server connection information
    $sql_details = array(
        ...
        $join = " inner JOIN user ON user.id = {$table}.userid   LEFT JOIN cart_shop ON cart_shop.number_bill={$table}.number_bill";
        $whereAll = array("");
    ...
    {
        $list_serial=array();
        $stmt = $this->db->prepare("SELECT  enter_serial   FROM `cart_shop` WHERE `number_bill` =? AND enter_serial <> ''");
