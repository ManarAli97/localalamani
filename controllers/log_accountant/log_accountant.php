...
// SQL server connection information
    ...
    function account_billAll($id,$date)
    {
        ...
        if ($date==null)
        {
            ...
            $stmt =$this->db->prepare("SELECT   `number_bill`  FROM  `cart_shop` WHERE  `user_direct`  = ? AND `id_accountant_user`=? AND  `accountant` = 1  AND cancel=0  GROUP BY `number_bill` ");
        ...
        }else
        {
            ...
            $stmt =$this->db->prepare("SELECT   `number_bill`  FROM  `cart_shop` WHERE  `user_direct`  = ? AND `id_accountant_user`=?  AND  `accountant` = 1  AND cancel=0 AND `date_accountant`  between ? AND ? GROUP BY `number_bill` ");
...
// SQL server connection information
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill` ,`id_member_r`,`prepared`,`dollar_exchange` FROM `cart_shop` WHERE `user_direct` =?  AND `accountant`=1 AND `number_bill`=? AND `number_bill` <> 0   AND cancel=0 GROUP BY `id_item`,`table`,`code`,`color`,`user_direct`,`number_bill` ORDER BY `id` DESC  ");
...
/*
    function check_amountDirect3($id)
    {
            ...
            $stmt =$this->db->prepare("SELECT   `number_bill`  FROM  `cart_shop` WHERE  `user_direct`  = ?  AND  `accountant` =0 GROUP BY `number_bill` ");
            $stmt->execute(array($id));
...
*/
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`date_req`,`mpx`,`number_bill`,`top`,`dollar_exchange` FROM `cart_shop` WHERE `user_direct` =? AND `number_bill`=?  AND   `accountant`=0 AND cancel=0 GROUP BY `id_item`,`table`,`code`,`color` ORDER BY `id` DESC  ");
