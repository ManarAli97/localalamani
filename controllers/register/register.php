        ...
        if (isset($_SESSION['username_member_r'])) {
           ...
           $dollar=0;
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $stmt1 = $this->db->prepare("UPDATE `cart_shop` SET `buy` = 1 ,`number_bill`=?,`dollar_exchange`=?,user_direct=? WHERE `id_member_r`=? AND `buy` = 0 ");
...
//            echo 'Mailer Error: ' . $mail->ErrorInfo;
    ...
    function processing_request($id)
    {
        if ($this->handleLogin()) {
            ...
            $stmt2 = $this->db->prepare("UPDATE `cart_shop` SET   `number_bill` = ? , `status` = 1 ,  `buy` = 2 , `date_d_r`= ? , `user_id`=? WHERE  `buy` = ?  AND `id_member_r`=?  AND `date_req` IN ($date_req)  ");
    ...
    function processing_request_edit_delivery_user($id)
    {
        if ($this->handleLogin()) {
            ...
            $stmt2 = $this->db->prepare("UPDATE `cart_shop` SET `number_bill` = ? ,`delivery_user` = ? , `status` = 1  , `user_id`=? WHERE  `buy` = ?  AND `id_member_r`=?  AND `date_d_r` =?  ");
    ...
    function processing_request_after_rej($id)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $stmt = $this->db->prepare("UPDATE `cart_shop` SET date_req=?, `status` = 1  , `buy` =2,   `user_id`=?  WHERE  `buy` = ? AND `id_member_r`=? AND  `date_d_r` =? ");
            $stmt->execute(array(time(), $_SESSION['userid'], 3, $id, $date_d_r));

            if ($stmt->rowCount() > 0) {

                $stmt1 = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`code`,`name_color`,`table`,SUM(`number`)as number,`buy`,`date` FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND `buy` =2 AND `date_d_r`=? GROUP BY `id_item`,`size`,`table`,`code` ORDER BY `id`  DESC  ");
    ...
    function processing_request_rejected($id)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $date_req = implode(',', $date_req);
            $stmt1 = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date` FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND `buy` = 1  AND  `date_req` IN ($date_req) GROUP BY `id_item`,`size`,`table`,`code` ORDER BY `id`  DESC  ");
            ...
            $stmt = $this->db->prepare("UPDATE `cart_shop` SET `status` = 2  , `buy` = 3, `why_rejected` = ?  , `date_d_r`= ? , `user_id`=?  WHERE  `buy` = ? AND `id_member_r`=? AND  `date_req` IN ($date_req)");
    ...
    function processing_request_rejected_after_order($id)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $stmt = $this->db->prepare("UPDATE `cart_shop` SET `date_req`=? ,`status` = 2  , `buy` = 3, `why_rejected` = ?  , `user_id`=?  WHERE  `buy` = ? AND `id_member_r`=? AND  `date_d_r` = ? AND `date_d_r` <> 0");
            $stmt->execute(array(time(), $why_rej, $_SESSION['userid'], 2, $id, $date_d_r));

            if ($stmt->rowCount() > 0) {
                $stmt1 = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date` FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND `buy` = 3 AND `date_d_r` = ? GROUP BY `id_item`,`size`,`table`,`code` ORDER BY `id`  DESC  ");
                $stmt1->execute(array($id, $date_d_r));
                while ($row = $stmt1->fetch(PDO::FETCH_ASSOC)) {
    ...
    function return_order_minus($table,$code,$id_user)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $stmt_count_n = $this->db->prepare("SELECT  *FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ?   AND `buy` = 1 AND `number` = 1");
            $stmt_count_n->execute(array($table, $code, $id_user,$color));

            if ($stmt_count_n->rowCount() > 0 )
            {

                $stmt_delete_last_one = $this->db->prepare("SELECT SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE  `table` =?  AND `code` =? AND `id_member_r` = ?   AND `name_color` = ?   AND `buy` = 1 ");
                $stmt_delete_last_one->execute(array($table, $code, $id_user,$color));
                   $number= $stmt_delete_last_one->fetch(PDO::FETCH_ASSOC)['number'];

                if ($number == 1) {

                    $stmt = $this->db->prepare("DELETE   FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `buy` = 1  LIMIT 1 ");
                    $stmt->execute(array($table, $code, $id_user,$color));
                    echo  0;

                }else if ($number > 1)
                    {
                    $stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` - 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?  AND `buy` = 1  AND number > 1 LIMIT  1  ");
			...
			if ($table == 'computer') {
                ...
                $stmt = $this->db->prepare("UPDATE   `{$this->cart_shop}` SET  `mpx`  =?   WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ?   AND `buy` = 1   ");
                $stmt->execute(array($mpx, $table, $code, $id_user,$color));



                $stmt2 = $this->db->prepare("SELECT  SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `name_color` = ? AND `buy`= 1 GROUP BY `id_item`,`size`,`table`,`code`,`name_color` ORDER BY `id`  DESC  ");
    ...
    function return_order_plus($table,$code,$id_user)
    {
        ...
        if ($this->handleLogin()) {
            ...
            $stmt_sel = $this->db->prepare("UPDATE  `cart_shop` SET  `number`=`number` + 1  WHERE `table` =?  AND `code` =? AND `id_member_r` = ? AND  `name_color` = ? AND `buy` = 1    LIMIT  1  ");
            $stmt_sel->execute(array($table, $code, $id_user,$color));
                ...
                if ($stmt_sel->rowCount() > 0 ) {
                    ...
                    if ($table == 'product_savers') {
                        $stmt_srv = $this->db->prepare("SELECT `color`,SUM(`number`)as number  FROM `{$this->cart_shop}` WHERE `table` =?  AND `code` =? AND `id_member_r` = ?  AND `buy`= 1 GROUP BY `id_item`,`size`,`table`,`code` ORDER BY `id`  DESC  ");
   ...
   public function notification_buy($id)
   {
    ...
    {
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE `buy` = 1 AND  `status` = 0  GROUP BY `id_member_r`");
        $stmt->execute();
        $count_active=array();
        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
        {
    ...
    public  function view_req2($id,$bill)
    {
        ...
        $infoOrder=$this->db->prepare("SELECT *FROM `cart_shop` WHERE `id_member_r`=? AND `number_bill`=?");
    ...
    public  function details_client($id,$id_user_direct=null)
    {
        ...
        if ($from_date_stm && $to_date_stm)
        {
            ...
            $stmt_date_req_done = $this->db->prepare("SELECT   * FROM `cart_shop` WHERE `id_member_r` =? AND date_req BETWEEN  ? AND ? AND   `buy` = 2 AND   `accountant` = 1 AND cancel=0    GROUP BY `number_bill`   ORDER BY `number_bill` DESC ");
            $stmt_date_req_done->execute(array($id,$from_date_stm,$to_date_stm));
        }else
        {
            $stmt_date_req_done = $this->db->prepare("SELECT  *  FROM `cart_shop` WHERE `id_member_r` =?  AND   `buy` = 2 AND   `accountant` = 1 AND cancel=0  GROUP BY `number_bill`    ORDER BY `number_bill` DESC ");
...
//            {
//
//                $stmt_date_req_done = $this->db->prepare("SELECT   * FROM `cart_shop` WHERE `id_member_r` =? AND date_req BETWEEN  ? AND ? AND   `buy` = 2 AND   `accountant` = 1 AND cancel=0  AND user_direct = ? GROUP BY `number_bill`   ORDER BY `number_bill` DESC ");
//                $stmt_date_req_done->execute(array($id,$from_date_stm,$to_date_stm,$id_user_direct));
//            }else
//            {
//                $stmt_date_req_done = $this->db->prepare("SELECT  *  FROM `cart_shop` WHERE `id_member_r` =?  AND   `buy` = 2 AND   `accountant` = 1 AND cancel=0  GROUP BY `number_bill` AND user_direct = ?  ORDER BY `number_bill` DESC ");
...
//        }
        ...
        while ($row_d =$stmt_date_req_done->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $xp1Offer=0;
            $stmt = $this->db->prepare("SELECT  cart_shop.* ,SUM(`number`)as number FROM `cart_shop` WHERE `id_member_r` =?  AND `number_bill`= ? AND   `buy` = 2 AND   `accountant` = 1 AND cancel=0 GROUP BY `id_item`,`table`,`code`,`number_bill`   ");
        ...
        if ($from_date_stm && $to_date_stm)
        {
            ...
            $stmt_date_req_done = $this->db->prepare("SELECT   * FROM `cart_shop` WHERE `id_member_r` =? AND date_req BETWEEN  ? AND ? AND   `buy` = 3 AND   cancel=1   GROUP BY `number_bill`   ORDER BY `number_bill` DESC ");
            $stmt_date_req_done->execute(array($id,$from_date_stm,$to_date_stm));
        }else
        {
            $stmt_date_req_done = $this->db->prepare("SELECT  *  FROM `cart_shop` WHERE `id_member_r` =?  AND   `buy` = 3 AND cancel=1  GROUP BY `number_bill`    ORDER BY `number_bill` DESC ");
            $stmt_date_req_done->execute(array($id));
        }
        ...
        while ($row_r =$stmt_date_req_done->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $stmt = $this->db->prepare("SELECT  cart_shop.* ,SUM(`number`)as number FROM `cart_shop` WHERE `id_member_r` =?  AND `number_bill`= ? AND   `buy` = 3  AND cancel=1 GROUP BY `id_item`,`table`,`code`,`number_bill`   ");
    ...
    public function all_active_buy()
    {
        ...
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE `buy` = 1 AND  `status` = 0 GROUP BY `id_member_r`  ORDER BY `date_req` ASC ");
        $stmt->execute();
        $count_active=array();
        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
        {
    ...
    {
        if (!is_numeric($id)) {$error=new Errors(); $error->index();}
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE `id` = ?  ");
        $stmt->execute(array($id));
        $result=$stmt->fetch(PDO::FETCH_ASSOC);
    ...
    function empty_car()
    {
        ...
        $stmt = $this->db->prepare( "DELETE FROM  cart_shop WHERE  `id_member_r` = ? AND accountant=0  AND `buy` = 0   ");
    ...
    function dlt_item($id,$id_c,$id_offer)
    {
        ...
        if ($id_offer)
        {
            ...
            $stmt = $this->db->prepare( "DELETE FROM {$this->cart_shop} WHERE  `id_member_r` = ?   AND `id_offer` = ? AND accountant=0  AND `buy` = 0   ");
            $stmt->execute(array($id_member_r,$id_offer));
            if ($stmt->rowCount() > 0)
            {

                $stmt_d = $this->db->prepare("SELECT  *FROM {$this->cart_shop}  WHERE   `id_member_r` = ?   AND `id_offer` = ? AND accountant=0  AND `buy` = 0  ");
            ...
            $stmt = $this->db->prepare( "DELETE FROM {$this->cart_shop} WHERE  `id_member_r` = ?   AND `id_item` = ?  AND `id`=? AND accountant=0  AND `buy` = 0");
            $stmt->execute(array($id_member_r,$id,$id_c));
            if ($stmt->rowCount() > 0)
            {

                $stmt_d = $this->db->prepare("SELECT  *FROM {$this->cart_shop}  WHERE   `id_member_r` = ?  AND AND accountant=0  `buy` = 0  ");
    ...
    function search_live()
    {
        ...
        $idU=implode(',',$idU);
        $stmt = $this->db->prepare("SELECT *FROM `cart_shop` WHERE `buy` = 1 AND  `status` = 0   AND `id_member_r` IN ($idU)   GROUP BY `id_member_r`  ORDER BY `date_req` ASC ");
        $stmt->execute();
        $count_active=array();
        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
        {
...
/*
    function chg()
    {
       ...
       $code=array(
        ...
        $cde = implode(',', $code);
        $stmt = $this->db->prepare("UPDATE `cart_shop` SET `delivered`=1, `delivered_date` = ?  WHERE  `buy` = 2 AND `status`=1 AND `number_bill` NOT IN ($cde)");
...

{
    ...
    public function processing_order_screen($fromdate_bill_timestamp=0,$todate_bill_timestamp=0,$bill1=null,$bill2=null)
    {
        ...
        $columns = array(
            ...
            array('db' => 'cart_shop.user_direct', 'dt' => 22),
        ...
        $join = " inner JOIN cart_shop ON cart_shop.id_member_r = register_user.id   LEFT JOIN user ON user.id =  cart_shop.user_direct ";
       if ($fromdate_bill_timestamp && $todate_bill_timestamp )
        {
            ...
            if ($bill1 ==1 && $bill2 ==1 )
            {
                $whereAll = array(" register_user.city <> '' ",
                    "  register_user.`xc`=1 ",
                    " ( cart_shop.`byqr`=0 OR   cart_shop.`byqr`=1   ) ",
                    "  cart_shop.`date_req` BETWEEN {$fromdate_bill_timestamp} AND {$todate_bill_timestamp} ",
                    " cart_shop.accountant =1"," cart_shop.cancel =0" ," cart_shop.prepared =2");
        ...
        else
        {
            ...
            if ($bill1  ==1  && $bill2  ==1 )
            {
                $whereAll = array("register_user.city <> '' ",
                    "  register_user.`xc` = 1 ",
                    " ( cart_shop.`byqr`=0 OR   cart_shop.`byqr`=1   ) ",
                    " register_user.gander <> ''  "," cart_shop.accountant =1"," cart_shop.cancel =0" ," cart_shop.prepared =2 ");
        ...
        $group=" GROUP BY  cart_shop.id_member_r,  cart_shop.user_direct ,  cart_shop.byqr  " ;
        echo json_encode(
    ...
    function sum_number_bill_user($id_user,$id_customer)
    {
        ...
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE  (`user_direct` = ? OR user_direct=0)   AND  id_member_r =?   AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2 GROUP BY   id_member_r");
        ...
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE (`user_direct` = ? OR user_direct=0)   AND  id_member_r =?   AND  `date_req` BETWEEN  ? AND  ?    AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2 GROUP BY   id_member_r");
            $stmt->execute(array($id_user,$id_customer,$_SESSION['fromdate_bill_timestamp'],$_SESSION['todate_bill_timestamp']));
        }else if (!empty( $_SESSION['fromdate_bill_timestamp']) &&  !empty( $_SESSION['todate_bill_timestamp'])   &&  !empty( $_SESSION['bill1'])   &&  empty( $_SESSION['bill2']) )
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE   id_member_r =? AND byqr = 0   AND  `date_req` BETWEEN  ? AND  ?    AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2 GROUP BY  id_member_r");
            $stmt->execute(array($id_customer,$_SESSION['fromdate_bill_timestamp'],$_SESSION['todate_bill_timestamp']));
        }
        else if (!empty( $_SESSION['fromdate_bill_timestamp']) &&  !empty( $_SESSION['todate_bill_timestamp'])   &&  empty( $_SESSION['bill1'])   &&  !empty( $_SESSION['bill2']) )
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE `user_direct` = ?  AND  id_member_r =? AND byqr = 1  AND direct <> 0  AND  `date_req` BETWEEN  ? AND  ?    AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2 GROUP BY   ,id_member_r");
            $stmt->execute(array($id_user,$id_customer,$_SESSION['fromdate_bill_timestamp'],$_SESSION['todate_bill_timestamp']));
        }

        else if (empty( $_SESSION['fromdate_bill_timestamp']) &&  empty( $_SESSION['todate_bill_timestamp'])   &&  !empty( $_SESSION['bill1'])   &&  !empty( $_SESSION['bill2']) )
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE   id_member_r =?     AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2  GROUP BY   id_member_r");
        ...
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE   id_member_r =? AND byqr=0    AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2 GROUP BY   id_member_r");
            $stmt->execute(array($id_customer ));
        }

        else if (empty( $_SESSION['fromdate_bill_timestamp']) &&  empty( $_SESSION['todate_bill_timestamp'])   &&  empty( $_SESSION['bill1'])   &&  !empty( $_SESSION['bill2']) )
        {
            $stmt = $this->db->prepare("SELECT  count(number_bill) as couts  FROM `cart_shop` WHERE  `user_direct` = ?  AND id_member_r =? AND byqr=1  AND direct <> 0    AND accountant =1  AND     cancel =0  AND  buy =2 AND   prepared =2 GROUP BY   id_member_r");
            $stmt->execute(array($id_user,$id_customer ));
        }
