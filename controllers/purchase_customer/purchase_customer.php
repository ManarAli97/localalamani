                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    while ($row=$stmt->fetch(PDO::FETCH_ASSOC))
                    {
                        ...
                        if ($row['serial'])
                        {
                            ...
                            {
                                $stmtGetSerial = $this->db->prepare("SELECT *FROM serial WHERE code =? AND model =?  ORDER BY  page DESC LIMIT 1");
                                $stmtGetSerial->execute(array($row['code'],$row['table']));
                                if ($stmtGetSerial->rowCount() > 0) {
                                  $resultSerial=  $stmtGetSerial->fetch(PDO::FETCH_ASSOC);
                                   $stmtSERial = $this->db->prepare("INSERT INTO `serial` ( page,bill, code, serial, type_enter, quantity, userId, `date`, model,note) VALUE (?,?,?,?,?,?,?,?,?,?) ");
                                    $stmtSERial->execute(array($resultSerial['page'], $resultSerial['bill'], $resultSerial['code'],$srl,$row['number_bill'].' فاتورة شراء ', 1, $resultSerial['userId'], time(), $resultSerial['model'],$row['number_bill'].' شراء  '));

                                }
                                else
                                {

                                    $stmtSERial = $this->db->prepare("INSERT INTO `serial_conform` ( code, model, type, userId, date) VALUE (?,?,?,?,?) ");
                                    $stmtSERial->execute(array($row['code'],$row['table'],$row['number_bill'].'  شراء رقم الفاتورة  ' , $this->userid ,time() ));
                        ...
                        if ($stmtChCode->rowCount() > 0) {
                            ...
                            $stmtExcel_archive = $this->db->prepare("INSERT INTO uesr_add_excel (code,quantity,price,type,number_bill,`date`,date_archives,username,wholesale_price,wholesale_price2,cost_price,model)  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
                            $stmtExcel_archive->execute(array($row['code'],$qty, $row['price_dollars'],'شراء','شراء',time(),time(),$this->userid,$row['wholesale_price'],$row['wholesale_price2'],$row['cost_price'],$row['table']));
                            ...
                            $stmtExcel->execute(array($row['code'],$qty, $row['price_dollars'], $this->min_price($row['price_dollars']), $this->max_price($row['price_dollars']),'شراء',time(),time(),$this->userid,$row['wholesale_price'],$row['wholesale_price2'],$row['cost_price']));
                        ...
                        if (!empty($row['location'])) {
                            ...
                            if ($stmt_ch_location->rowCount() > 0) {
                                $stmtExcel_conform = $this->db->prepare("UPDATE location SET  quantity=quantity + {$qty} ,`date`=?  WHERE code =? AND  model=? AND location=?");
                                $stmtExcel_conform->execute(array(time(), $row['code'], $row['table'], trim($row['location'])));
                            } else {
                                ...
                                $stmtExcel_conform = $this->db->prepare("INSERT INTO location (sequence,quantity,`date`,code,model,location) VALUES (?,?,?,?,?,?)");
                                $stmtExcel_conform->execute(array($sequence,$qty, time(), $row['code'], $row['table'], trim($row['location'])));
                            }
                        }else
                        {
                            ...
                            if ($stmtChCodeConform->rowCount() > 0) {
                                $stmtExcel_conform = $this->db->prepare("UPDATE location_confirm SET  quantity=quantity+ {$qty} ,price_dollars=?,`date`=?  WHERE code =? AND  model=?");
                                $stmtExcel_conform->execute(array($row['price_dollars'],time(), $row['code'],$row['table']));

                            }else
                            {

                                $stmtExcel_conform = $this->db->prepare("INSERT INTO  location_confirm (quantity,code,price_dollars,model,date)  values (?,?,?,?,?)");
                                $stmtExcel_conform->execute(array($qty,$row['code'],$row['price_dollars'],$row['table'],time()));
