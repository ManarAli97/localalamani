...
// SQL server connection information
    ...
    {
        $stmt = $this->db->prepare("SELECT  `date_req`,`date_d_r`,`id_member_r`,`delivery_user`,`number_bill` ,`dollar_exchange` FROM `cart_shop` WHERE `id_accountant_user` =?  AND  `accountant`=1 AND cancel=0 GROUP BY `number_bill`   ORDER BY `id` DESC ");
    ...
    {
        $stmt = $this->db->prepare("SELECT  cart_shop.*,SUM(`number`)as number  FROM `cart_shop` WHERE `id_accountant_user` =?  AND   `accountant` = 1   AND cancel=0   AND `number_bill`= ? AND cancel=0 GROUP BY `id_item`,`table`,`code`,`number_bill`  ORDER BY `id` DESC  ");
