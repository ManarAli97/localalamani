        ...
        if (isset($_POST['submit']))
        {
            try
            {
                ...
                if (empty($this->error_form)) {
                    $stmt = $this->db->prepare("INSERT INTO `{$this->table}` (`title`,`id_cat`,`content`, `description` ,`date`,`bast_it`,`serial_flag`,`price_dollars`,`tags`,`class_games`,`specifications`,`cuts`,`price_cuts`,`location`,`enter_serial`,`change_price`,`userId`) VALUE (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->execute(array($data['title'], $data['id_cat'], $data['content'],$data['description'], $data['date'], $data['bast_it'],$data['serial_flag'],$data['price_dollars'], $data['tags'], $class_games, $specifications, $data['cuts'], $data['price_cuts'], $data['location'], $data['enter_serial'], $data['change_price'],$this->userid));
        ...
        if (isset($_POST['submit']))
        {
            ...
            try
            {
                ...
                if ($this->permit('save_edit',$this->folder)) {
                    if (empty($this->error_form)) {
                        $stmt = $this->db->prepare("UPDATE   `{$this->table}` SET  `title`=? ,  `content`=? ,`description`=? , `id_cat`=? ,`date`=? ,`bast_it`=? ,`serial_flag`=? ,`price_dollars`=?,`tags`=?,`class_games`=?,`specifications`=?,`cuts`=?,`price_cuts`=?,`location`=?,`enter_serial`=?,`change_price`=?,`userId`=? WHERE `id`=?");
                        $stmt->execute(array($data['title'], $data['content'],$data['description'], $data['id_cat'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $class_games, $specifications, $data['cuts'], $data['price_cuts'], $data['location'],$data['enter_serial'],$data['change_price'],$this->userid, $id));
    ...
    function class_games($id,$id_cat=null)
    {
        ...
        }else
        {
           ...
           $id_category=implode(',',$id_category);
            ...
            $stmt=$this->db->prepare("SELECT {$this->table}.id, {$this->table}.price_dollars,{$this->table}.title,{$this->table}.price_cuts,{$this->table}.bast_it,{$this->table}.cuts,{$this->table}.description  FROM {$this->table} INNER JOIN {$this->color} ON  {$this->color}.id_item={$this->table}.id INNER JOIN {$this->code} ON {$this->code}.id_color= {$this->color}.id INNER JOIN {$this->excel} ON {$this->excel}.code = {$this->code}.code WHERE   {$this->table}.`id_cat` IN({$id_category})  AND   {$this->table}.`class_games`  REGEXP ? AND  {$this->table}.`active` = 1  AND {$this->is_delete}   group by   {$this->table}.`id`  ORDER BY {$this->table}.`id` DESC  ");
            $stmt->execute(array($q));
        }
