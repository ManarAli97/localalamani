    ...
    function byqr()
    {
        ...
        if ($this->handleLogin()) {
            ...
            if (!empty($result)) {
                if ($_SESSION['direct'] == 3)
                {
                    $stmt_uuid = $this->db->prepare("UPDATE `cart_shop` SET `date`=?,`id_member_r` = ? ,`direct`=? ,`user_direct`=?,`prepared` = ?,`dollar_exchange`=? ,byqr=1 WHERE `id_member_r`=? AND `buy` = 0 AND `status`=0");
                    $stmt_uuid->execute(array(time(),$id_user, $_SESSION['direct'], $this->userid,1,$dollar, $uuid));
                }else{
                    $stmt_uuid = $this->db->prepare("UPDATE `cart_shop` SET `date`=?,`id_member_r` = ? ,`direct`=? ,`user_direct`=?,`dollar_exchange`=? ,byqr=1 WHERE `id_member_r`=? AND `buy` = 0 AND `status`=0");
                    $stmt_uuid->execute(array(time(),$id_user, $_SESSION['direct'], $this->userid,$dollar, $uuid));
                ...
                $stmt1 = $this->db->prepare("UPDATE `cart_shop` SET `buy` = 1 ,`number_bill`=?,byqr=1 WHERE `id_member_r`=? AND `buy` = 0 ");
                $stmt1->execute(array($number_bill, $id_user));
