...
// SQL server connection information
	...
	public function processing_bills_crystal_enter($from_date_stm=null,$to_date_stm=null)
	{
		...
		$columns = array(
			...
			array('db' => $this->table.'.crystal_bill', 'dt' => 6),
			...
			array('db' => $this->table.'.id', 'dt' => 7,
				'formatter' => function ($d, $row) {
             	 ...
             	 <div class='row justify-content-center align-items-center'>
				  <div class='col-auto' style='padding-left: 2px'>		
				  	   <input id='numberBill_{$row[14]}{$row[10]}' value='{$row[6]}'  onkeyup=''  type='text' class='form-control withBill' name='crystal_bill' required>
                    </div>
				  <div class='col-auto'  style='padding-right: 2px'>
				   <button type='submit' id='btn_in_bill_{$row[14]}{$row[10]}' onclick=saveBill('" . $row[14] . "','" . $row[10] . "')  name='submit' class='btn  btn-success'>حفظ</button>
                  </div>
                  </div>
				 ";
						} else {
							return "
             	              <div class='row justify-content-center align-items-center'>
				  <div class='col-auto' style='padding-left: 2px'>		
				  	       <input id='numberBill_{$row[14]}{$row[10]}'  value='{$row[6]}' type='text' class='form-control withBill' name='crystal_bill' required>
                   </div>
				  <div class='col-auto' style='padding-right: 2px'>
			...
			array('db' => $this->table.'.number_bill', 'dt' => 8,
				'formatter' => function ($d, $row) {
			...
			array('db' =>$this->table.'.id', 'dt' => 9),//11
			array('db' =>$this->table.'.id_member_r', 'dt' =>10),//12
			array('db' =>$this->table.'.user_direct', 'dt' =>11),//14
			array('db' =>$this->table.'.cancel', 'dt' =>12),//15
 			array('db' =>$this->table.'.number_bill', 'dt' =>13),//17
             array('db' =>$this->table.'.edit_price', 'dt' =>14),//17
...
// SQL server connection information
	...
	public function bills_crystal_not_enter()
	{
        ...
        $stmt = $this->db->prepare("SELECT number_bill FROM `cart_shop` WHERE  accountant = 1 AND prepared=2 AND crystal_bill  = '' AND  cancel = 0  AND  group_bill = ''  GROUP BY number_bill,id_member_r") ;
        $stmt->execute();
        $bill=array();
		...
		$columns = array(
			...
			array('db' => $this->table.'.number_bill', 'dt' => 3,
			    ...
			    if ($row[14]==1)
                {
                    if ($row[12] == 1)
                    {
                        return   $this->sunAmountBillByNumberBill($row[9],$d) .' = <span style="background: red">تم الغاء الفاتورة </span>';
                ...
                }else
                {
                    if ($row[12] == 1)
                    {
                        ...
                        return $this->sunAmountBillByNumberBill($row[9],$d);
			...
			array('db' => $this->table.'.number_bill', 'dt' => 6,
				'formatter' => function ($d, $row) {
					return "<button  class='btn btn-secondary' onclick='get_details($row[9],$d)'  >{$d}</button>";
				}
			),
			...
			array('db' => $this->table.'.id', 'dt' => 7,
				'formatter' => function ($d, $row) {
             	 ...
             	 <div class='row justify-content-center align-items-center'>
				  <div class='col-auto' style='padding-left: 2px'>		
				  	       <input id='numberBill_{$row[13]}{$row[9]}'  value='' type='text' class='form-control withBill' name='crystal_bill' required>
                   </div>
				  <div class='col-auto' style='padding-right: 2px'>
				   <button type='submit' id='btn_in_bill_{$row[13]}{$row[9]}}' onclick=saveBill('".$row[13]."','".$row[9]."')  name='submit' class='btn btn-warning'>حفظ</button>
                  </div>
                  </div>
                    " ;
					}else
			...
			array('db' =>$this->table.'.id', 'dt' => 8),//8
			array('db' =>$this->table.'.id_member_r', 'dt' =>9),//9
			array('db' =>$this->table.'.id_accountant_user', 'dt' =>10),//10
			array('db' =>$this->table.'.user_direct', 'dt' =>11),//11
			array('db' =>$this->table.'.cancel', 'dt' =>12),//12
			array('db' =>$this->table.'.number_bill', 'dt' =>13),//14
            array('db' =>$this->table.'.edit_price', 'dt' =>14),//17
...
// SQL server connection information
		...
		if (empty($from_date_stm) && empty($to_date_stm))
		{
			$whereAll = array("cart_shop.accountant = 1 ","cart_shop.crystal_bill  = '' "," cart_shop.cancel = 0 "," cart_shop.group_bill = '' ");

		}else{
			$whereAll = array("cart_shop.accountant = 1 ","cart_shop.crystal_bill  = '' "," cart_shop.cancel = 0 "," cart_shop.group_bill = '' ","cart_shop.date_req BETWEEN {$from_date_stm} AND {$to_date_stm}");
...
// SQL server connection information
		...
		$join = "inner JOIN register_user ON register_user.id = cart_shop.id_member_r left JOIN crystal_bill ON crystal_bill.number_bill = cart_shop.number_bill left JOIN user ON user.id = cart_shop.id_accountant_user ";
		 ...
		 $whereAll = array("crystal_bill.delete =0","cart_shop.date_req BETWEEN {$from_date_stm} AND {$to_date_stm}","crystal_bill.edit = 1 ");
...
// SQL server connection information
		...
		$join = "inner JOIN register_user ON register_user.id = cart_shop.id_member_r inner JOIN crystal_bill ON crystal_bill.number_bill = cart_shop.number_bill left JOIN user ON user.id = cart_shop.id_accountant_user  ";

			$whereAll = array("crystal_bill.delete =0","crystal_bill.checked = 1","cart_shop.date_req BETWEEN {$from_date_stm} AND {$to_date_stm}");
...
// SQL server connection information
		...
		$join = "inner JOIN register_user ON register_user.id = cart_shop.id_member_r inner JOIN crystal_bill ON crystal_bill.number_bill = cart_shop.number_bill left JOIN user ON user.id = cart_shop.id_accountant_user  ";
			...
			$whereAll = array("crystal_bill.delete =0","cart_shop.date_req BETWEEN {$from_date_stm} AND {$to_date_stm}","crystal_bill.checked = 0","crystal_bill.edit = 0");
...
// SQL server connection information
	...
	public function sumNomberBill($id,$n_b)
	{
	...
	function sunAmountBillByNumberBill($id,$n_b)
	{
...
function all_note_enter_bill()
{
	...
	$stmt = $this->db->prepare("SELECT COUNT(t.row) as num FROM (SELECT  count(id) as row  FROM `cart_shop`    WHERE  accountant = 1 AND crystal_bill  = '' AND  cancel = 0  AND  group_bill = ''    GROUP BY  number_bill,id_member_r) as t") ;
	$stmt->execute();
	if ($stmt->rowCount() >0)
    {
        $result=$stmt->fetch(PDO::FETCH_ASSOC);
    ...
    function add_group()
    {
        ...
        if (!empty($bill)) {
            ...
            foreach ($bill as $data)
            {
                ...
                $stmt_cart_shop = $this->db->prepare("UPDATE  `cart_shop` SET  group_bill=?   WHERE  number_bill =?   ");
...
//
//        $stmt = $this->db->prepare("SELECT  cart_shop.number_bill FROM `cart_shop`  left JOIN crystal_bill ON crystal_bill.number_bill = cart_shop.number_bill    WHERE   cart_shop.number_bill not in(SELECT number_bill FROM group_bill) AND  cart_shop.accountant = 1 AND  cart_shop.prepared = 2 AND cart_shop.cancel=0 AND  crystal_bill.number_bill is null GROUP BY cart_shop.number_bill") ;
//        $stmt->execute();
//        $bill=array();
//        while ($row =$stmt->fetch(PDO::FETCH_ASSOC))
//        {
        ...
        $columns = array(
            ...
            array('db' => $this->table.'.number_bill', 'dt' => 6,
                'formatter' => function ($d, $row) {
                   ...
                   return "{$this->customerInfo($row[8])} ,{$this->UserInfo($row[8],'c')},{$this->UserInfo($row[9])},{$this->UserInfo($row[10])}   {$this->UserInfo($row[8])},{$d},{$this->details_offer($row[13],'title')},{$this->info_price_type($row[14])},{$date}";
                }
...
// SQL server connection information
        ...
        if (empty($from_date_stm) && empty($to_date_stm))
        {
            $whereAll = array("cart_shop.number_bill IN({$ids})");

        }else{
            $whereAll = array("cart_shop.date_req BETWEEN {$from_date_stm} AND {$to_date_stm}","cart_shop.number_bill IN({$ids})");
        }

        $group="GROUP BY  cart_shop.id_item, cart_shop.code, cart_shop.table,cart_shop.id_member_r,cart_shop.number_bill";
...
// SQL server connection information
    ...
    function info_price_type($num)
    {
        ...
        function sum_quantity($number_bill,$id_member,$id_item,$table,$code)
        {
            ...
            $stmt = $this->db->prepare("SELECT SUM(number) as number FROM `cart_shop`   where  number_bill =? AND  id_member_r =? AND id_item=? AND `table`=? AND `code` = ? ") ;
                 ...
                 if ($stmt_ch->rowCount() > 0) {
                    ...
                    {$time}
                         ...
                         if ($stmt_in->rowCount() > 0) {
                             ...
                             foreach ($bill as $data) {
                                 $stmt_cart_shop = $this->db->prepare("UPDATE  `cart_shop` SET  crystal_bill=?   WHERE  number_bill =?   ");
    ...
    {
        $stmt = $this->db->prepare("SELECT  cart_shop.number_bill FROM `cart_shop`   where  number_bill =? AND  prepared =1 ") ;
    ...
    public function export_serial($number)
    {
        ...
        $stmtc = $this->db->prepare("SELECT     id,code,enter_serial FROM  cart_shop    WHERE   cart_shop.number_bill IN({$ids})  AND  `enter_serial`  <> '' ") ;
        $stmtc->execute();
        $serial=array();
...
// SQL server connection information
        ...
        if (empty($from_date_stm) && empty($to_date_stm))
        {
            $whereAll = array( "cart_shop.crystal_bill  = ' ' "," cart_shop.cancel = 1   ");

        }else{
            $whereAll = array( "cart_shop.crystal_bill  = ' ' "," cart_shop.cancel = 1 ", "cart_shop.date_req BETWEEN {$from_date_stm} AND {$to_date_stm}");
