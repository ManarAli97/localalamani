...
trait search
{
    ...
    function data()
    {
        ...
        $stmt = $this->db->prepare("SELECT  cart_shop.*,SUM(`number`)as number FROM `cart_shop` WHERE  `number_bill`=?   GROUP BY `id_member_r`,`id_item`,`table`,`code`,`number_bill`,price_type,id_offer  ");
        ...
        if ($request) {
            ...
            $stmtOffer = $this->db->prepare("SELECT  cart_shop.*  FROM `cart_shop` WHERE  `number_bill`=?  AND id_offer <> 0 AND offers = 'offers' GROUP BY  `date_offer`   ");
            $stmtOffer->execute(array($number_bill));
            ...
            $stmtOffer = $this->db->prepare("SELECT  cart_shop.*,SUM(number) as number FROM `cart_shop` WHERE  `number_bill`=?    AND id_offer = 0  AND offers = ''  GROUP BY  `id_item`,`table`,`code`,`color`,`number_bill`,price_type    ");
            $stmtOffer->execute(array($number_bill));
