...
class printing_supplies extends Controller
{
    ...
    public function admin_category($id=0)
    {
        ...
        $data=$this->db->select("SELECT * from  {$this->category} WHERE  `relid` = {$id} ");
    ...
    function getLoopIdX($id)
    {
        ...
        $stmt=$this->db->prepare("SELECT *from  {$this->category} WHERE  `relid` = {$id} AND `active` = 1 ");
    ...
    public  function list_all($id=null,$page=1)
    {
        ...
        $this->ids[]=$id;
        $stmt=$this->db->prepare("SELECT *from  {$this->category} WHERE  `relid` = {$id} AND `active` = 1 ORDER BY `id` DESC  ");
        $stmt->execute(array($id));
        while ($row=$stmt->fetch(PDO::FETCH_ASSOC))
        {
            $this->ids[]=$row['id'];
        ...
        $result=$this->db->select("SELECT * from  {$this->category} WHERE  `id` = {$id} AND `active` = 1");
        $result=$result[0];
        $stmt=$this->db->prepare("SELECT *FROM `$this->table` WHERE `id_cat` IN ($ids_cat) AND `active` = 1");
    ...
    public  function list_view($id=null,$page=1)
    {
        ...
        $stmt_specif=$this->db->prepare("SELECT *FROM `specifications`  WHERE `model`='printing_supplies'");
        $stmt_specif->execute();
...
// SQL server connection information
    ...
    {
        $stmt=$this->db->prepare("SELECT * from  {$this->category} WHERE `relid`=?");
        $stmt->execute(array($id));
        return $stmt;
    }
    ...
    public function sub_cat_active($r_id,$id=null)
    {
        ...
        $stmt=$this->db->prepare("SELECT * from  `$this->category`  WHERE `id`=? ");
         $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
            $result=$stmt->fetch(PDO::FETCH_ASSOC);
...
//-----------------category------------------
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->category}` WHERE  `active` =? AND `relid`=0 ORDER BY `id` ASC ");
    ...
    {
        $stmt=$this->db->prepare("SELECT * from  `$this->category` WHERE `relid`=?  AND `active`=1 ORDER BY `order_cat` ASC");
        $stmt->execute(array($id));
        return $stmt;
    }
    ...
    {
        $stmt=$this->db->prepare("SELECT * from   `$this->category`  WHERE `relid`=?  AND `active`=1");
        $stmt->execute(array($id));
        if ($stmt->rowCount() > 0)
        {
            return true;
 ...
 function getAllItemByIdCat($id,$limit=5)
 {
    ...
    function car_item($id,$count)
    {
        ...
        if (empty($error))
        {
            ...
            if ($stmt_ch->rowCount() > 0) {
                ...
                $stmt_order = $this->db->prepare("SELECT   SUM(`number`)as num,`dollar_exchange`  FROM `cart_shop` WHERE `code` =?  AND  (`buy` = 0 OR `buy` = 1) AND `status` = 0    AND `table`=?  AND  `id_member_r` = ?");
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND   `buy` = 1 ORDER BY `id` DESC  ");
        $stmt->execute(array($id_member_r));
        return $stmt;
    }
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND `buy` = 2 ORDER BY `id` DESC  ");
        $stmt->execute(array($id_member_r));
        return $stmt;
    }
    ...
    public function filter()
    {
        ...
        if (isset($_REQUEST['specifications'])) {
            ...
            }else
            {
                ...
                $stmt = $this->db->prepare("SELECT *FROM `{$this->table}` WHERE `id_cat`  IN ({$Id_cat})  AND   ({$fieldDetails})  AND  `active` = 1 AND {$this->bast_it} ORDER BY `id` DESC  ");
...
// SQL server connection information
        ...
        {
            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
            $whereAll = array("{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}");
        }else if (is_numeric($id)  && empty($_GET['from_price']) && empty($_GET['to_price']) )
        {
            ...
            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
            $whereAll = array("{$this->table}.id_cat IN({$ids})");
        }else   if (is_numeric($id)  && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            $ids=implode(',', $this->getLoopId($id));

            $join = " inner JOIN {$this->color} ON {$this->code}.id_color = {$this->color}.id  inner JOIN {$this->table} ON {$this->color}.id_item = {$this->table}.id LEFT JOIN {$this->excel} ON {$this->code}.code = {$this->excel}.code ";
            $whereAll = array("{$this->table}.id_cat IN({$ids})","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}");
        }
...
// SQL server connection information
    ...
    function load_desc($id=null)
    {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            usort($idItemC, function ($item1, $item2) {
