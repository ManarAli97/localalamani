...
// SQL server connection information
        ...
        {
            $whereAll = array("{$table}.active = 2","{$table}.date BETWEEN {$from_date_stm} AND {$to_date_stm}","number_group = 0 ","isdelete = 0","location.sequence between {$from} AND  {$to}");
        ...
        }else    if (empty($from_date_stm) && empty($to_date_stm) && !empty($id_gl))
        {
            ...
            $whereAll = array("{$table}.active = 2","number_group = 0","isdelete = 0","location.sequence between {$from} AND  {$to}");
        }else{
            $whereAll = array("{$table}.active = 2","{$table}.date BETWEEN {$from_date_stm} AND {$to_date_stm}","number_group = 0 ","isdelete = 0");
