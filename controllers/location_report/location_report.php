...
//		}
	...
	public function processing_report($from=0,$to=0,$model='mobile',$id=null,$fromDate=0,$toDate=0)
	{
		...
		$columns = array(
			...
			array(  'db' => 'location.model', 'dt'=>3,
				'formatter' => function( $d, $row ) {
					return $this->sum_quantity_location($row[2],$d);
				}
			),
			...
			array(  'db' => 'location.model', 'dt'=>4,
				'formatter' => function( $d, $row ) {
					...
					if ($this->fromDate > 0 && $this->toDate > 0){
						$stmt=$this->db->prepare("SELECT  SUM(`number`) as num  FROM cart_shop WHERE code=? AND `table`=?  AND  date_prepared between  ? AND ?");
						$stmt->execute(array($row[2],$d,$this->fromDate, $this->toDate));
					}else
					{

						$stmt=$this->db->prepare("SELECT  SUM(`number`) as num  FROM cart_shop WHERE code=? AND `table`=?");
						$stmt->execute(array($row[2],$d));
					}
			...
			array(  'db' =>'location.model', 'dt'=>5,
				'formatter' => function( $d, $row ) {
					return $this->table_location($row[2],$d,$this->from);
				}
			),
			array(  'db' =>'location.model', 'dt'=>6,
				'formatter' => function( $d, $row ) {
					return $this->table_location($row[2],$d,$this->to);
				}
			),
...
//			}
	...
	public function processing_report_accessories($from=0,$to=0,$model='accessories',$id=null,$fromDate=0,$toDate=0)
	{
		...
		$columns = array(
			...
			array(  'db' => $color.'.maximum', 'dt'=>3),
			array(  'db' => $color.'.minimum', 'dt'=>4),
			array(  'db' => 'location.model', 'dt'=>5,
				'formatter' => function( $d, $row ) {
					return $this->sum_quantity_location($row[2],$d);
				}
