            ...
            if (!isset($_SESSION['usernamelogin']))
            {
                ...
                if (isset($_COOKIE['g_active']))
                {
                    ...
                    if ($stmt_login->rowCount() > 0)
                    {
                        ...
                        $stmtx = $this->db->prepare("SELECT   `id_member_r`  FROM  `cart_shop` WHERE  `user_direct`  = ?  AND number_bill = 0 GROUP BY `user_direct` ");
...
//        }
        ...
        if (isset($_SESSION['username_member_r'])  ||  $this->isDirect())
        {
            ...
            $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,   `number` ,`buy`,`date`,`id_offer`,`offers` FROM `cart_shop` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY  `id_offer`,`offers`,`date` ORDER BY `id`  DESC  ");
...
//    }
    ...
    function row_cart()
    {
        ...
        $stmt=$this->db->prepare("SELECT *FROM cart_shop WHERE  id_member_r=? AND code =? AND id_item =? AND `table`=? AND `name_color`=? AND  prepared=0 LIMIT 1" );
    ...
    function note_cart_note()
    {
        ...
        $stmt=$this->db->prepare("UPDATE cart_shop SET note=?  WHERE  id_member_r=? AND code =? AND id_item =? AND `table`=? AND `name_color`=? AND  prepared=0 LIMIT 1" );
...
//            $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date`,`id_offer`,`offers` FROM `cart_shop` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY  `id_offer`,`offers`,`id_item`,`size`,`table`,`price`,`name_color` ORDER BY `id`  DESC  ");
            $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,   `number` ,`buy`,`date`,`id_offer`,`offers`,`price_type` FROM `cart_shop` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY  `id_offer`,`offers`,`date` ORDER BY `id`  DESC  ");
            $stmt->execute(array($id));
