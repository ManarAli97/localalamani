...
class computer extends Controller
{
    ...
    public function createTB()
    {
        ...
        return  $this->db->cht(array($this->table, $this->category, $this->color, $this->code, $this->excel,$this->like_computer));
                ...
                if ($this->permit('save_edit',$this->folder)) {
                    ...
                    foreach ($name_color as $key => $save_data) {
                        ...
                        foreach ($code[$key] as $index => $cd) {
                            ...
                            $stmt_cd->execute(array($index, $cd, $size[$key][$index],$point[$key][$index], $serial[$key][$index],  $key_id, time()));
                        }
                    }
						$trace=new trace_site();
    ...
    function delete_computer($id)
    {
        if ($this->handleLogin()) {
            ...
            $ids_color = array();
         	...
         	$stmt_codes_sync=$this->db->prepare("SELECT code FROM `$this->code` where id_color in (select id from `$this->color` where id_item =? )");
        ...
        if ($this->handleLogin()) {
            ...
            if ($stmt_item->rowCount() > 0) {
                while ($row_item = $stmt_item->fetch()) {
                    ...
                    $stmt_codes = $this->db->prepare("SELECT code FROM  `$this->code` where id_color in (select id from `$this->color` where id_item =? )");
