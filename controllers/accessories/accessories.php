...
class Accessories extends Controller
{
    ...
    function dtl($id,$price_dollars)
    {
        ...
        $stmt = $this->db->prepare("SELECT {$this->excel}.*  from `{$this->color}` INNER JOIN `{$this->excel}` ON `{$this->excel}`.code=`{$this->color}`.code  WHERE  `{$this->color}`.`id`=? AND   {$this->is_delete}  AND `{$this->excel}`.`quantity` > 0  ");
        $stmt->execute(array($id));
        if ($stmt->rowCount()>0)
        {
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function filter()
    {
        ...
        if (isset($_REQUEST['specifications'])) {
            ...
            if ($id_cat=='all')
            {
                ...
                $stmt = $this->db->prepare("SELECT *FROM `{$this->table}` WHERE  ({$fieldDetails})   AND  `active` = 1 AND  {$this->is_delete} AND {$this->bast_it} ORDER BY `id` DESC  ");
                $stmt->execute();
            }else
            {
...
// SQL server connection information
    ...
    function search()
    {
        if ($this->handleLogin()) {
            $data = $_GET['q'];
            $data = '%' . $data . '%';
            $stmt = $this->db->prepare("SELECT * FROM `category_accessories`  WHERE title LIKE ?  AND  {$this->is_delete}   ");
    ...
    function accessories_info($id)
    {
        if ($this->handleLogin()) {
            ...
            $stmt = $this->db->prepare("SELECT `title` FROM `category_accessories` WHERE  `id`=? AND  {$this->is_delete}  ");
            $stmt->execute(array($id));
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            echo $result['title'];
        }
    ...
    {
        $stmt = $this->db->prepare("SELECT `{$this->color}`.*,  {$this->excel}.price_dollars, {$this->excel}.price , {$this->excel}.wholesale_price , {$this->excel}.wholesale_price2 , {$this->excel}.cost_price  FROM `{$this->color}`  INNER JOIN {$this->excel} ON {$this->excel}.code = `{$this->color}`.code    WHERE   `{$this->color}`.`id_item`= ? AND `{$this->color}`.`is_delete`= 0 AND {$this->excel}.quantity > 0  GROUP BY {$this->excel}.code LIMIT 1");
        $stmt->execute(array($id));
        return $stmt;
    }
