...
class Accessories extends Controller
{
    ...
    public function details($id)
    {
        ...
        $id_c = 0;
        $count = 0;
        $g_c = $this->db->prepare("SELECT color_accessories.*,excel_accessories.quantity FROM `color_accessories` INNER JOIN  excel_accessories ON excel_accessories.code = color_accessories.code  WHERE   color_accessories.`id_item`= ? AND excel_accessories.quantity > 0 ");
    ...
    {
        $stmt = $this->db->prepare("SELECT id,img,code,`code_color`,`color` FROM `{$this->color}` WHERE   `id`= ?   LIMIT $limit ");
        $stmt->execute(array($id));
    ...
    {
        $stmt = $this->db->prepare("SELECT *FROM `{$this->color}` WHERE   `id_item`= ?   ");
        $stmt->execute(array($id));
        return $stmt;
    }
    ...
    function dtl($id,$price_dollars)
    {
        ...
        $stmt = $this->db->prepare("SELECT {$this->excel}.*  from `{$this->color}` INNER JOIN `{$this->excel}` ON `{$this->excel}`.code=`{$this->color}`.code  WHERE  `{$this->color}`.`id`=? AND `{$this->excel}`.`quantity` > 0  ");
        $stmt->execute(array($id));
        if ($stmt->rowCount()>0)
        {
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if (isset($_POST['submit']))
        {
            try
            {
                ...
                if (empty($this->error_form)) {
                    $stmt = $this->db->prepare("INSERT INTO `{$this->table}` (`title`,`id_cat`,`content`,`description`,`date`,`bast_it`,`serial_flag`,`price_dollars`,`tags`,`cuts`,`price_cuts`,`specifications`,`location`,`enter_serial`,`change_price`,`userId`) VALUE (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->execute(array($data['title'], $data['id_cat'], $data['content'],$data['description'], $data['date'], $data['bast_it'],$data['serial_flag'],$data['price_dollars'], $data['tags'], $data['cuts'], $data['price_cuts'], $specifications, $data['location'], $data['enter_serial'], $data['change_price'],$this->userid));
        ...
        if (isset($_POST['submit']))
        {
			...
			try
            {
                ...
                if (empty($this->error_form)) {
                    ...
                    $stmt = $this->db->prepare("UPDATE   `{$this->table}` SET  `title`=? ,  `content`=? ,  `description`=? , `id_cat`=? ,`date`=? ,`bast_it`=? ,`serial_flag`=? ,`price_dollars`=?,`tags`=? ,`cuts`=? ,`price_cuts`=? ,`specifications`=? ,`location`=?  ,`enter_serial`=?  ,`change_price`=?  ,`userId`=? WHERE `id`=?");
                    $stmt->execute(array($data['title'], $data['content'],$data['description'], $data['id_cat'], $data['date'], $data['bast_it'], $data['serial_flag'], $data['price_dollars'], $data['tags'], $data['cuts'], $data['price_cuts'], $specifications, $data['location'],$data['enter_serial'],$data['change_price'], $this->userid,$id));
...
// SQL server connection information
	...
	function details_acc($id)
	{
		...
		$stmt=$this->db->prepare("SELECT color_accessories.*,excel_accessories.quantity FROM `color_accessories` left   JOIN excel_accessories ON color_accessories.code=excel_accessories.code WHERE color_accessories.id_item=? ");
		$stmt->execute(array($id));
		$html="
    ...
    function delete_accessories($id)
    {
        if ($this->handleLogin()) {
        	...
        	$stmt_codes_sync=$this->db->prepare("SELECT code FROM `color_accessories` where id_item =?");
...
//-----------------category------------------
    ...
    {
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`,SUM(`number`)as number,`buy`,`date` FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY `id_item`,`size`,`table`,`price`,`name_color` ORDER BY `id`  DESC  ");
            ...
            if (empty($error))
            {
                ...
                if ($stmt_ch->rowCount() > 0) {
                    ...
                    if ($q >= $data['number']) {
						...
						$stmt_chx = $this->db->prepare("SELECT   *FROM `cart_shop` WHERE `id_item` =?  AND `code` =?  AND  `buy` = 0 AND `status` = 0    AND `table`=?  AND  `id_member_r` = ?  AND `name_color`=? AND  price_type=0 ");
						$stmt_chx->execute(array($data['id_item'],$data['code'], $this->table, $data['id_member_r'], $data['name_color']));
						if ($stmt_chx->rowCount() > 0)
						{
							$stmtUpdate_cart=$this->db->prepare("UPDATE `cart_shop` SET `number`=number+? WHERE `id_item` =?  AND `code` =?  AND  `buy` = 0 AND `status` = 0    AND `table`=?  AND  `id_member_r` = ?   AND `name_color`=? AND  price_type=0 ");
           ...
           $data['date'] = time();
            ...
            if ($stmt_ch->rowCount() > 0) {
                ...
                if ($q >= $data['number']) {
					...
					$stmt_chx = $this->db->prepare("SELECT   * FROM `cart_shop` WHERE `id_item` =?  AND `code` =?  AND  `buy` = 0 AND `status` = 0    AND `table`=?  AND  `id_member_r` = ? AND `name_color`=?  AND  price_type=? ");
					$stmt_chx->execute(array($data['id_item'],$data['code'], $this->table, $data['id_member_r'], $data['name_color'], $data['price_type']));
					if ($stmt_chx->rowCount() > 0)
					{
						$stmtUpdate_cart=$this->db->prepare("UPDATE `cart_shop` SET `number`=number+1 WHERE `id_item` =?  AND `code` =?  AND  `buy` = 0 AND `status` = 0    AND `table`=?  AND  `id_member_r` = ? AND `name_color`=?  AND  price_type=?  ");
    ...
    function count_c()
    {
        ...
        $stmt = $this->db->prepare("SELECT `id`, `id_item`,`size`,`price`,`price_dollars`,`image`,`color`,`name_color`,`code`,`table`, `number`,`buy`,`date` FROM `{$this->cart_shop}` WHERE `id_member_r` =?  AND `buy` = 0 GROUP BY `offers`,`id_offer`,`date` ORDER BY `id`  DESC  ");
        $stmt->execute(array($id));
            $car=array();
            $count=0;
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
...
// SQL server connection information
    ...
    public function processing_quantity($id)
    {
        ...
        $columns = array(
            ...
            array('db' =>'color_accessories.color', 'dt' => 2),

            array('db' => $tableJoin . 'code', 'dt' =>3),
            array('db' => 'excel_accessories.quantity', 'dt' => 4),
            array('db' => 'excel_accessories.price_dollars', 'dt' => 5),

            array('db' =>'accessories.location', 'dt' => 6,
                'formatter' => function( $d, $row ) {
                    if ($d == 1) {
                        $span = "<span class='location_active_{$row[8]}' style='color: green;font-weight: bold;display: block'>ON</span>";
                    } else {
                        $span = "<span class='location_active_{$row[8]}' style='color: red;font-weight: bold;display: block'>OFF</span>";
                    ...
                    if ($this->switch_location==0)
                    {
                        if ($this->permit('visible_location', $this->folder)) {
                            $span .= "
                            <div style='text-align: center'>
                              <input {$this->ch_location($row[8])} class='toggle-demo' onchange='visible_accessories_location(this,$row[8])' type='checkbox' data-on='On' data-off='Off' id='toggle-event'    data-toggle='toggle' data-style='ios' data-onstyle='success' data-size='small'>
                             </div>
                         ";
                        }
...
// SQL server connection information
        ...
        {
            $join = " inner JOIN accessories ON color_accessories.id_item = accessories.id INNER JOIN category_accessories ON category_accessories.id = accessories.id_cat LEFT JOIN excel_accessories ON color_accessories.code = excel_accessories.code ";
            $whereAll = array("accessories.$this->is_delete");
        }else if ($id=='all' && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            $join = " inner JOIN accessories ON color_accessories.id_item = accessories.id INNER JOIN category_accessories ON category_accessories.id = accessories.id_cat LEFT JOIN excel_accessories ON color_accessories.code = excel_accessories.code ";
            $whereAll = array("accessories.$this->is_delete","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}");
        }else if (is_numeric($id)  && empty($_GET['from_price']) && empty($_GET['to_price']) )
        {
            ...
            $join = " inner JOIN accessories ON color_accessories.id_item = accessories.id INNER JOIN category_accessories ON category_accessories.id = accessories.id_cat LEFT JOIN excel_accessories ON color_accessories.code = excel_accessories.code ";
            $whereAll = array("accessories.$this->is_delete","{$this->table}.id_cat IN({$ids})");
        }else   if (is_numeric($id)  && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            $ids=implode(',', $this->getLoopId($id));

            $join = " inner JOIN accessories ON color_accessories.id_item = accessories.id INNER JOIN category_accessories ON category_accessories.id = accessories.id_cat LEFT JOIN excel_accessories ON color_accessories.code = excel_accessories.code ";
            $whereAll = array("accessories.$this->is_delete","{$this->table}.id_cat IN({$ids})","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}");
        }
...
// SQL server connection information
        ...
        if ($id=='all' && empty($_GET['from_price']) && empty($_GET['to_price']) )
        {
            ...
            $whereAll = array("accessories.$this->is_delete","( color_accessories.minimum <> '' OR  color_accessories.maximum <> '')"," excel_accessories.quantity < color_accessories.minimum ");
        }else if ($id=='all' && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            ...
            $whereAll = array("accessories.$this->is_delete","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}","( color_accessories.minimum <> '' OR  color_accessories.maximum <> '' )"," excel_accessories.quantity < color_accessories.minimum ");
        }else if (is_numeric($id)  && empty($_GET['from_price']) && empty($_GET['to_price']) )
        {
            ...
            $join = " inner JOIN accessories ON color_accessories.id_item = accessories.id   LEFT JOIN excel_accessories ON color_accessories.code = excel_accessories.code ";
            $whereAll = array("accessories.$this->is_delete","{$this->table}.id_cat IN({$ids})","( color_accessories.minimum <> '' OR  color_accessories.maximum <> '' )"," excel_accessories.quantity < color_accessories.minimum ");
        }else   if (is_numeric($id)  && !empty($_GET['from_price']) && !empty($_GET['to_price']) )
        {
            $ids=implode(',', $this->getLoopId($id));

            $join = " inner JOIN accessories ON color_accessories.id_item = accessories.id   LEFT JOIN excel_accessories ON color_accessories.code = excel_accessories.code ";
            $whereAll = array("accessories.$this->is_delete","{$this->table}.id_cat IN({$ids})","{$this->excel}.price_dollars BETWEEN {$_GET['from_price']} AND {$_GET['to_price']}","( color_accessories.minimum <> '' OR  color_accessories.maximum <> '' )"," excel_accessories.quantity < color_accessories.minimum ");
        }
...
// SQL server connection information
    ...
    function search()
    {
        if ($this->handleLogin()) {
            $data = $_GET['q'];
            $data = '%' . $data . '%';
            $stmt = $this->db->prepare("SELECT * FROM `category_accessories`  WHERE title LIKE ?    ");
