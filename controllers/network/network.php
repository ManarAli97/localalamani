...
class network extends Controller
{
    ...
    public  function list_all($id=null,$page=1)
    {
        ...
        $this->ids[]=$id;
        $stmt=$this->db->prepare("SELECT *from  {$this->category} WHERE  `relid` = ? AND `active` = 1   ORDER BY `id` DESC  ");
        $stmt->execute(array($id));
        while ($row=$stmt->fetch(PDO::FETCH_ASSOC))
        {
            $this->ids[]=$row['id'];
    ...
    public  function list_view($id=null,$page=1)
    {
        ...
        $stmtCat=$this->db->prepare("SELECT *FROM {$this->category}  WHERE `active` = 1 ");
        $stmtCat->execute();
        $catRange=array();
