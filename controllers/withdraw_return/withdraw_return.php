...
class withdraw_return extends Controller
{
    ...
    function smart_prepared()
    {
       ...
       $stmt=$this->db->prepare("SELECT  cart_shop.*, SUM(number) as number   FROM cart_shop WHERE id_member_r=? AND number_bill=? AND  accountant=1 AND  prepared=1 AND  cancel=0 AND wr_prepared=0 GROUP BY id_item,code,`table` ");
...
//
//
//                               $stmt_cart_shop = $this->db->prepare("UPDATE `cart_shop` SET    `buy` = 2 ,  `prepared` = 2 , `date_prepared`= ? , `user_direct`=?,location=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=?  AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
...
//
                           ...
                           }else
                           {
                               ...
                               {
                                   $locationFoundrtrim=implode(',',$locationFound);
                                   $stmt_cart_shop = $this->db->prepare("UPDATE `cart_shop` SET  wr_prepared=?,id_prepared=?,location=? WHERE `code`=? AND `id_member_r` =? AND `table` = ? AND `id_item`=?  AND `number_bill`=?  AND `accountant`=1 AND `prepared`=1 ");
